...
on error resume next
 ...
 Select Case bootT
        Case 0: 
   D1.text = "CREATED BY IVANTBA"
   DisplayB2SText "CREATED BY IVANTBA"
   Playsound "fx_sys11_bootup"
        Case 1: 
   D1.text = "HIGHLANDER 1986"
   DisplayB2SText "HIGHLANDER 1986"
   Playsound "fx_sys11_bootup"
        Case 2: 
   D1.text = "THE PINBALL"
   DisplayB2SText "THE PINBALL"
   Playsound "fx_sys11_bootup"
        Case 3:
   bootT = 0
   BootE = False
...
sub droptarget7_hit
    addscore(4000)
    addbonus(400)
    PlaySoundAt "crash2", DropTarget7
...
sub animatebonus()
    ...
    DisplayB2SText "  TOTAL BONUS   " & " "&(Bonus)
end sub
...
sub messagetimer_Timer()
   ...
   ' player2reel.text = "Dr"
    ...
    DisplayB2SText2 "    DEFEAT THE     " & "   KURGAN   "
    end if
    ...
    DisplayB2SText2 "    PLAY THE    " & "  HIGHLANDER "
    end if
    ...
    DisplayB2SText2 "    PRESENTS   " & "   BY IVANTBA  "
    end if
    ...
    D1.text = "COLLECT SWORDS"
    'player2reel.text = "by"
    D2.text = "EVERYWHERE"
    'player4reel.text = "Bailey"
    DisplayB2SText2 " COLLECT SWORDS  " & "   EVERYWHERE   "
    end if
   ...
   ' player4reel.text = "START"
    ...
    D1.text = "HIGHLANDER"
    D2.text = "THE PINBALL"
    DisplayB2SText2 "    HIGHLANDER   " & "   THE PINBALL  "
    End If

    If attractmessage = 13 then
    D1.text = "CONVERSION BY"
    'player2reel.text = " "&Credits
    D2.text = "JAVIER"
   ' player4reel.text = "Start"
    DisplayB2SText2 " CONVERSION BY  " & "   JAVIER       "  
    End If
...
Sub ShowHighScores_Timer
	...
	D1.text = "THE SWORD MASTER" 
	D2.text = " 1> " & SpecialHighScoreName(0) & ":  " & SpecialHighScore(0)

    DisplayB2SText2 "THE SWORD MASTER"& " 1> " & SpecialHighScoreName(0) & ":" & SpecialHighScore(0)
	Case 6:

	D1.text = "THE SWORD MASTER" 
	D2.text = " 2> " & SpecialHighScoreName(1) & ":  " & SpecialHighScore(1)

    DisplayB2SText2 "THE SWORD MASTER"& " 2> " & SpecialHighScoreName(1) & ":" & SpecialHighScore(1)
	Case 7:

	D1.text = "THE SWORD MASTER"
	D2.text = " 3> " & SpecialHighScoreName(2) & ":  " & SpecialHighScore(2) 

    DisplayB2SText2 "THE SWORD MASTER"& " 3> " & SpecialHighScoreName(2) & ":" & SpecialHighScore(2)
    ...
    DisplayB2SText2 "THE SWORD MASTER"& " 4> " & SpecialHighScoreName(3) & ":" & SpecialHighScore(3)
...
sub showmessage()
    ...
    DisplayB2SText "    YOU ARE     " & "   IMMORTAL   "
    End If
    If Message = 5 then
    D1.text = "**EXTRABALL**"
	 'player2reel.text = "Ball**"
...
Sub AwardSpecial()
    'DMD " ", "EXTRA GAME", 2000
    D1.text = "EXTRA GAME"& "    CREDITS:  "& Credits
    PlaySound "fx_knocker"
    Credits = Credits + 1
...
Sub CheckSpecialHighscore()
    ...
    D1.Text = " "
    D2.Text = " "
    DisplayB2SText "SWORDS COLLECTED " & "                "&SpecialScore(CurrentPlayer)
    Dim tmp
    tmp = SpecialScore(1)
...
Sub SpecialHighScoreDisplayName()
    ...
    TempStr = TempStr & "< "
   ' DMDMod "hsc", "YOUR NAME:", Mid(TempStr, 2, 5), 999999
    DisplayB2SText "SWORD MASTER NAME" & Mid(TempStr, 2, 5)
    D1.TEXT = "SWORD MASTER NAME"
    D2.TEXT = "    " & Mid(TempStr, 2, 5)
End Sub
...
Sub SpecialHighScoreCommitName()
    ...
    DisplayB2SText " CONGRATULATIONS " & SpecialScore(CurrentPlayer)&"           SWORDS"
    LightEffect 2
    FlashEffect 2
    RingReward = True
