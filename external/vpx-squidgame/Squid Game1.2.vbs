...
cPuPPack = "men in black11"    ' name of the PuP-Pack / PuPVideos folder for this table
...
sub K3G_hit()
	K3G.destroyball
    'PlaySound "" : pupevent 833
	PyramidKicker.createball
	PyramidKicker.kick 180,0
end sub
...
sub Kickerdisk_hit()
    Kickerdisk.destroyball: CheckTargets 
	Timer001.enabled=1
    Lightlock.State = 0
    Lightlock1.State = 0
    PlaySound "threetwoon"
    ChangeBall(0)
...
sub timer001_timer
	Timer001.enabled=0
    KickerMIB.createball : DMD CL("MIB"), CL("MULTIBALL"), "", eBlink, eBlink, eNone, 3000, True, ""
	KickerMIB.kick 180,5
    ...
    FlashForMs Fspot, 2000, 50, 0
end sub

sub K1D_hit()
	K1D.destroyball : DMD CL("MIB"), CL("MULTIBALL"), "", eBlink, eBlink, eNone, 3000, True, "fx_fanfare1"
    AddMultiball 2
...
Sub target15_Hit 
    Target15Hit = 1: CheckTargets
    Lightlock.State = 2 : PlaySound "fouet"
    Lightdisk.State = 2
...
Sub Targetlock_Hit 
    TargetLockHit = 1: CheckTargets
    Lightdisk1.State = 2
    Lightlock1.State = 2 : PlaySound "fouet"
    FlashForMs FvertD, 1000, 50, 0
    FlashForMs FvertG, 1000, 50, 0
End Sub

Sub CheckTargets
If Target15Hit  + TargetLockHit = 2 Then
    Kickerdisk.Enabled = 1
End If
End Sub
...
Sub CheckMIB()
    If LightM.State + LightI.State + LightB.State = 3 Then
       LowerRamp
       Lcroco.State = 2
       Lcrocoa.State = 2
    'TargetM.IsDropped = 0
...
Sub EndOfBallComplete()
    ...
    If((BallsRemaining(CurrentPlayer) <= 0)AND(BallsRemaining(NextPlayer) <= 0))Then
        ...
        EndOfGame() :Playsound "gameover" : pupevent 859
...
Sub DMDScore()
    ...
    if(dqHead = dqTail)Then
        ...
        Select Case Battle(CurrentPlayer, 0)
            ...
            Case 9:tmp1 = CL("HIT ALVO TARGET " & 5-CaptiveBallHits)
            ...
            Case 15:tmp1 = CL("THE BEST OF MIB" & "Ball " & Balls)
        End Select
        tmp2 = ""
    End If
...
Sub ShowTableInfo
    ...
    End If
    DMD "", CL("LETS PLAY A GAME"), "", eNone, eBlink, eNone, 2000, False, ""
    If bFreePlay Then
    ...
    Else
        ...
        Else
            DMD CL("CREDITS " & Credits), CL("INSERT CURRENCY"), "", eNone, eBlink, eNone, 2000, False, ""
        End If
    End If
    DMD "      RICARDO ", "      PRESENT", "d_jppresents", eNone, eNone, eNone, 3000, False, ""
    DMD "", "", "d_title", eNone, eNone, eNone, 4000, False, ""
    DMD "", CL("   RICARDO ATTILI " &myversion), "", eNone, eNone, eNone, 2000, False, ""
...
Sub Check10BankTargets
     If light12.state + Light12a.state + light13.state + Light13a.state + light14.state + LightE.state + LightL.state + LightT.state + LightO.state + LightN.state = 10 Then
        light12.state = 0
        light12a.state = 0
        light13.state = 0
        Light13a.state = 0
        ...
        Addscore 50000 : PlaySound "letter_laser"
        LightEffect 1
        FlashEffect 2
        Addscore 30000
...
Sub Check4BankTargets
     If LightJ.state + LightO1.state + LightH.state + LightN1.state = 4 Then
        ...
        LightH2.state = 2
        LightN2.state = 2
        Addscore 100000 : PlaySound "letter_laser"
        LightEffect 1
        FlashEffect 2
    End If
End Sub
...
Sub Check2BankTargets
    If light11.state + light10.state = 2 Then
        ...
        If(Light27.State = 0)AND(bMultiballMode = FALSE)Then 'lit the lock light if it is off, rise the lock post and activate the lock switch
            ...
            DMD "_", CL("LOCK IS LIT"), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_lockislit" : pupevent 813
...
Sub lock_Hit
    ...
    If light27.State = 1 Then 'lock the ball
        ...
        Select Case BallsInLock(CurrentPlayer)
            Case 1:DMD "_", CL("BALL 1 LOCKED"), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_oneballlock" : Lock1.State = 1 : pupevent 814
            Case 2:DMD "_", CL("BALL 2 LOCKED"), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_twoballlock" : Lock2.State = 1 : pupevent 815
            Case 3:DMD "_", CL("BALL 3 LOCKED"), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_threeballlock" : Lock3.State = 1 : pupevent 816
        End Select
        light27.State = 0
...
Sub StartMainMultiball
    AddMultiball 3
    DMD "_", CL("MULTIBALL"), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_multiball" : Lock1.State = 0 : Lock2.State = 0 : Lock3.State = 0 : pupevent 831
    StartJackpots
    ChangeGi blue
...
Sub GiveRandomAward() 'from the Pyramid
    ...
    DMD CL("MIB AWARD"), "", "", eNone, eNone, eNone, 50, False, "fx_spinner"
...
Sub RightRampDone_Hit
    ...
    If light31.State = 2 Then
        DMD CL("RAMP CROCO"), CL(jackpot(CurrentPlayer)), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_Jackpot"
        AddScore Jackpot(CurrentPlayer)
        LightEffect 2
        FlashEffect 2
    Else
...
Sub StartBattle
    ...
    Select Case NewBattle
        Case 1 'Serpent Yards = Super Spinners
            DMD CL("SQUID GAME"), CL("SHOOT THE SPINNERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light45a.State = 2
            Light70.State = 2
            SpinCount = 0
        ...
        Case 2 'Calakmul = Super Pop Bumpers
            DMD CL("SQUID GAME"), CL("HIT THE POP BUMPERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light22.State = 2
            LightSeqBumpers.Play SeqRandom, 10, , 1000
            SuperBumperHits = 0
            pupevent 840
        Case 3 'Sierra de Chiapas = Ramps
            DMD CL("SQUID GAME"), CL("SHOOT THE RAMPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits3 = 0
            pupevent 847
        Case 4 'Karnak = Orbits
            DMD CL("SQUID GAME"), CL("SHOOT THE ORBITS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            OrbitHits = 0
            pupevent 837
        Case 5 'CopÃ¡n = Shoot the lights 2
            DMD CL("SQUID GAME"), CL("SHOOT THE LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light45.State = 2
            Light46.State = 2
            Light47.State = 2
        ...
        Case 7 'Palenque=  Target Frenzy
            DMD CL("SQUID GAME"), CL("ALL TARGETS-RAMPS"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqAllTargets.Play SeqRandom, 10, , 1000
            TargetHits7 = 0
            pupevent 838
        Case 8 'ChichÃ©n ItzÃ¡ = Left & Right Targets
            DMD CL("SQUID GAME"), CL("TARGETS AND RAMPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light50.State = 2
            Light50a.State = 2
            TargetHits8 = 0
        ...
        Case 9 'Bonampak = Captive Ball
            DMD CL("SQUID GAME"), CL("SHOOT ALVO TARGET"), "", eNone, eNone, eNone, 1500, True, ""
            Light29.State = 2
            CaptiveBallHits = 0
            pupevent 844
        Case 10 'Monte Alban = Ramps and Orbits
            'uses the ramphits10 to count the hits
            DMD CL(""), CL("SHOOT RAMPS AND LANE"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits10 = 0
            pupevent 839
        Case 11 'Uxmal = Blue Targets
            DMD CL("SQUID GAME"), CL("SHOOT BLUE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqBlueTargets.Play SeqRandom, 10, , 1000
            TargetHits11 = 0
            pupevent 849
        Case 12 'Valley of the Jaguar = Super Loops
            DMD CL("SQUID GAME"), CL("SHOOT THE LOOPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            Gate2.Open = 1
            Gate3.Open = 1
        ...
        Case 13 'The Grand Cathedral = Follow the Lights 1
            DMD CL("SQUID GAME"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
        ...
        Case 15 'Mordekai the Summoner - the final battle
            DMD CL("THE BEST SQUID GAME"), CL("SHOOT THE JACKPOTS"), "", eNone, eNone, eNone, 1500, True, ""
            AddMultiball 4
            StartJackpots
            ChangeGi green
            pupevent 852
...
Sub StopBattle2
    ...
    Select Case NewBattle
        ...
        Case 12:Gate2.Open = 0:Gate3.Open = 0:wallLoop.Collidable = 0
