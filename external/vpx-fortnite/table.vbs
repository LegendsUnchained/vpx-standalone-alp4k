...
' Global variables to manage song list and count
Dim songsList()
Dim remainingSongs

Sub InitializeSongs()
    ' Initialize the list of songs and remaining songs
    ReDim songsList(11)
    Dim i
    For i = 0 To 11
        songsList(i) = i
    Next
    remainingSongs = 12 ' Total number of songs
End Sub

' Initialize the songs and start the music
InitializeSongs
MusicOn

Sub MusicOn()
    Dim x, songIndex
    If remainingSongs = 0 Then
        InitializeSongs ' Reset if all songs have been played
    End If

    ' Select a random index from the remaining songs
    x = INT(remainingSongs * RND(1))
    songIndex = songsList(x)
    
    ' Play the selected song
    Select Case songIndex
        Case 0: PlayMusic "Fortnite_Boss.mp3"
        Case 1: PlayMusic "Fortnite_Banger.mp3"
        Case 2: PlayMusic "Fortnite_Bounce.mp3"
        Case 3: PlayMusic "Fortnite_Coral.mp3"
        Case 4: PlayMusic "Fortnite_OG.mp3"
        Case 5: PlayMusic "Fortnite_Shark.mp3"
        Case 6: PlayMusic "Fortnite_STW.mp3"
        Case 7: PlayMusic "Fortnite_Butter.mp3"
        Case 8: PlayMusic "Fortnite_Sax.mp3"
        Case 9: PlayMusic "Fortnite_Limitless.mp3"
        Case 10: PlayMusic "Fortnite_Spies.mp3"
        Case 11: PlayMusic "Fortnite_Long.mp3"
    End Select

    ' Remove the played song from the songsList
    Dim i
    For i = x To remainingSongs - 2
        songsList(i) = songsList(i + 1)
    Next
    ReDim Preserve songsList(remainingSongs - 2)
    remainingSongs = remainingSongs - 1
End Sub
