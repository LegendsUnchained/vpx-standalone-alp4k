...
'                 Guardians of galaxy Pinball Script JPSalas & marty
'                          Version 4.0
' ****************************************************************
...
Const BallMass = 0.8     ' 1 is the normal mass
...
Sub SphereTimer_Timer
   ...
   spherelord.roty = spherelord.roty + 5
end sub
...
Sub RollingUpdate()
    ...
    starlord.RotY = 120 - (BOT(2).Y)\15
    rocket.RotY = 160 - (BOT(2).Y)\15
...
Sub DMDScore()
    ...
    if(dqHead = dqTail)Then
        ...
        Select Case Battle(CurrentPlayer, 0)
            ...
            Case 2:tmp1 = CL("HIT BUMPERS CITY " & 25-SuperBumperHIts)
            Case 3:tmp1 = CL("HIT RAMPS " & 6-ramphits3)
            Case 4:tmp1 = CL("HIT LANE L-R " & 6-orbithits)
            Case 5:tmp1 = CL("HIT THE LIGHTS ")
            Case 6:tmp1 = CL("HIT THE LIGHTS ")
            Case 7:tmp1 = CL("HIT THE TARGETS " & 20-TargetHits7)
            Case 8:tmp1 = CL("HIT GREEN TARGETS " & 10-TargetHits8)
            Case 9:tmp1 = CL("HIT RED TARGET " & 6-CaptiveBallHits)
            Case 10:tmp1 = CL("HIT RAMPS ORBITS " & 6-RampHits10)
            Case 11:tmp1 = CL("HIT BLUE TARGETS " & 10-TargetHits11)
            Case 12:tmp1 = CL("HIT HYPER LOOPS " & 5-loopCount)
            Case 13:tmp1 = CL("HIT THE LIT LIGHT")
            Case 14:tmp1 = CL("HIT THE LIT LIGHT")
            Case 15:tmp1 = CL("STARLORD MISSION" & "Ball " & Balls)
        End Select
        tmp2 = ""
    End If
...
Sub ShowTableInfo
    ...
    DMD "", CL("I AM GROOT " &myversion), "", eNone, eNone, eNone, 2000, False, ""
...
Sub Trees_Timer()
    ...
    avion.RotY = - TreeDir *10
    groot.RotY = - TreeDir *15
    ronan.RotY = - TreeDir *15
End Sub
...
' Move Bumper
Dim Movebumper1Pos, Movebumper2Pos, Movebumper3Pos

Sub Movebumper1
    Movebumper1Pos = 8
    bullebump1timer.Enabled = 1
End Sub

Sub bullebump1timer_Timer
    bullebump1.TransX = Movebumper1Pos
    If Movebumper1Pos = 0 Then Me.Enabled = 0:Exit Sub
    If Movebumper1Pos < 0 Then
        Movebumper1Pos = ABS(Movebumper1Pos)- 1
    Else
        Movebumper1Pos = - Movebumper1Pos + 1
    End If
End Sub

Sub Movebumper2
    Movebumper2Pos = 8
    bullebump2timer.Enabled = 1
End Sub

Sub bullebump2timer_Timer
    bullebump2.TransX = Movebumper2Pos
    If Movebumper2Pos = 0 Then Me.Enabled = 0:Exit Sub
    If Movebumper2Pos < 0 Then
        Movebumper2Pos = ABS(Movebumper2Pos)- 1
    Else
        Movebumper2Pos = - Movebumper2Pos + 1
    End If
End Sub

Sub Movebumper3
    Movebumper3Pos = 8
    bullebump3timer.Enabled = 1
End Sub

Sub bullebump3timer_Timer
    bullebump3.TransX = Movebumper3Pos
    If Movebumper3Pos = 0 Then Me.Enabled = 0:Exit Sub
    If Movebumper3Pos< 0 Then
        Movebumper3Pos = ABS(Movebumper3Pos)- 1
    Else
        Movebumper3Pos = - Movebumper3Pos + 1
    End If
End Sub
...
Sub Check2BankTargets
    If light11.state + light10.state = 2 Then
        ...
        If(Light27.State = 0)AND(bMultiballMode = FALSE)Then 'lit the lock light if it is off, rise the lock post and activate the lock switch
            ...
            DMD "_", CL("LOCK IS LIT"), "_", eNone, eBlinkFast, eNone, 1000, True, "lockislit"
...
Sub Check5BankTargets
    ...
    If light15.state + light16.state + light17.state + light18.state + light19.state = 5 Then
        ...
        DMD "YOU HAVE FOUND", "THE OBJECTS", "monster_" &tmp, eNone, eNone, eNone, 2500, True, ""
        LightEffect 1
        FlashEffect 1
...
Sub GiveRandomAward() 'from the Pyramid
    ...
    DMD CL("STARLORD AWARD"), "", "", eNone, eNone, eNone, 50, False, "fx_spinner"
...
Sub LeftRampDone_Hit
    ...
    if LastSwitchHit = "LeftRampDone" Then
        Addscore jackpot(CurrentPlayer)
        DMD CL("COMBO"), CL(jackpot(CurrentPlayer)), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_combo"
    End If
...
Sub RightRampDone_Hit
    ...
    If light31.State = 2 Then
        DMD CL("BLASTER PETER"), CL(jackpot(CurrentPlayer)), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_Jackpot"
        AddScore Jackpot(CurrentPlayer)
        LightEffect 2
        FlashEffect 2
    Else
...
Sub StartBattle
    ...
    Select Case NewBattle
        Case 1 'Serpent Yards = Super Spinners
            DMD CL("LE MILANO"), CL("SHOOT THE SPINNERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            SpinCount = 0
        Case 2 'Calakmul = Super Pop Bumpers
            DMD CL("PETER QUILL"), CL("HIT THE POP BUMPERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light22.State = 2
            LightSeqBumpers.Play SeqRandom, 10, , 1000
            SuperBumperHits = 0
        Case 3 'Sierra de Chiapas = Ramps
            DMD CL("GAMORA"), CL("SHOOT THE RAMPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits3 = 0
        Case 4 'Karnak = Orbits
            DMD CL("COSMO"), CL("SHOOT THE ORBITS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            Light45a.State = 2
            OrbitHits = 0
        Case 5 'CopÃ¡n = Shoot the lights 2
            DMD CL("YONDU"), CL("SHOOT THE LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light45.State = 2
            Light46.State = 2
            Light47.State = 2
        ...
        Case 7 'Palenque=  Target Frenzy
            DMD CL("GROOT"), CL("SHOOT BLUE LIGHT"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqAllTargets.Play SeqRandom, 10, , 1000
            TargetHits7 = 0
        Case 8 'ChichÃ©n ItzÃ¡ = Left & Right Targets
            DMD CL("KRAGLIN"), CL("SHOOT THE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light50.State = 2
            TargetHits8 = 0
        Case 9 'Bonampak = Captive Ball
            DMD CL("ROCKET"), CL("SHOOT RED TARGET"), "", eNone, eNone, eNone, 1500, True, ""
            Light29.State = 2
            CaptiveBallHits = 0
        Case 10 'Monte Alban = Ramps and Orbits
            'uses the ramphits10 to count the hits
            DMD CL("MANTIS"), CL("SHOOT RAMPS ORBITS"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits10 = 0
        Case 11 'Uxmal = Blue Targets
            DMD CL("DRAX"), CL("SHOOT BLUE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqBlueTargets.Play SeqRandom, 10, , 1000
            TargetHits11 = 0
        Case 12 'Valley of the Jaguar = Super Loops
            DMD CL("NEBULA"), CL("SHOOT THE LOOPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            Gate2.Open = 1
            Gate3.Open = 1
        ...
        Case 13 'The Grand Cathedral = Follow the Lights 1
            DMD CL("STARLORD"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            FollowTheLights.Enabled = 1
            AddMultiball 2
            StartJackpots
            ChangeGi green
        Case 14 'The City of the Gods = Follow the Lights 2
            DMD CL("DRAX DESTTUCTEUR"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            FollowTheLights.Enabled = 1
            AddMultiball 2
            StartJackpots
            ChangeGi green
        Case 15 'Mordekai the Summoner - the final battle
            DMD CL("GAMORA"), CL("SHOOT THE JACKPOTS"), "", eNone, eNone, eNone, 1500, True, ""
            AddMultiball 4
            StartJackpots
            ChangeGi green
    End Select
...
Sub CheckWinBattle
    ...
    Select Case NewBattle
        ...
        Case 5
            If Light44.State + Light45.State + Light45a.State + Light46.State + Light47.State + Light48.State + Light49.State + Light50.State = 0 Then WinBattle:End if
        Case 6
        Case 7
