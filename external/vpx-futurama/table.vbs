...
'----- Mechanical tilt -----
Const MechTiltDebounce = 1000		'1000 miliseconds
...
' 1.2.2 (wb) :    Improved PuP performance
'                 Fixed a PuP issue where the last game's scores would not appear
'                 Ball save will now work in the right outlane even if all suicide booth saves have been used
'                 The suicide booth is no longer active after a tilt
'                 Added a debounce timer for mechanical tilt
...
Dim nTimeMechTilt		' Timestamp of when mechanical tilt was last triggered
			...
			if nDMDTextDisplayTime >= nAttractModePageTime Then
				...
				if bEnablePuP Then
					...
					ElseIf 1 = nAttractModePage Then
						...
						pDMDLabelSetAttract "PrevScores", "PREVIOUS GAME"
						For i = 1 to 4
							If i <= nPlayersLastGame Then
								pDMDLabelSetAttract "PrevP" & i & "Label", "PLAYER " & i
								pDMDLabelSetAttract "PrevP" & i & "Score", FormatScore(anScore(i - 1))
							Else
								pDMDLabelSetAttract "PrevP" & i & "Label", ""
								pDMDLabelSetAttract "PrevP" & i & "Score", ""
							End If
						Next
					Elseif 2 = nAttractModePage Then
						pDMDSetPage pAttract
						pDMDLabelSetAttract "Attract1", "FREE PLAY"
						pDMDLabelSetAttract "Attract2", "PRESS START"
					...
					Elseif 7 = nAttractModePage Then
						pDMDSetPage pAttract
						pDMDLabelSetAttract "Attract1", HighScoreName(4) & " " & FormatScore(HighScore(4))
						pDMDLabelSetAttract "Attract2", "HIGH SCORE 4"
					End If
				End If
...
Sub pSetPageLayouts
	...
	PuPlayer.LabelSet pDMD, "DeliverValue", " 0 ", 0, _
		"{'mt':2,'shadowcolor':0, 'shadowstate': 1, 'xoffset': 5, 'yoffset': 4, 'outline':1}"
	PuPlayer.LabelSet pDMD, "BonusX", " BONUS X 1 ", 0, _
		"{'mt':2,'shadowcolor':0, 'shadowstate': 1, 'xoffset': 5, 'yoffset': 4, 'outline':1}"
	PuPlayer.LabelSet pDMD, "BonusTotal", " BONUS TOTAL 0 ", 0, _
		"{'mt':2,'shadowcolor':0, 'shadowstate': 1, 'xoffset': 5, 'yoffset': 4, 'outline':1}"
	PuPlayer.LabelSet pDMD, "BonusScoreCaption", " PLAYER 1 TOTAL SCORE ", 0, _
		"{'mt':2,'shadowcolor':0, 'shadowstate': 1, 'xoffset': 5, 'yoffset': 4, 'outline':1}"
	PuPlayer.LabelSet pDMD, "BonusScoreValue", " 00 ", 0, _
		"{'mt':2,'shadowcolor':0, 'shadowstate': 1, 'xoffset': 5, 'yoffset': 4, 'outline':1}"
...
Sub pDMDLabelSetAttract(labName,LabText)
	PuPlayer.LabelSet pDMD, labName, "" & LabText & "", 1, ""
end sub
...
Sub Table1_KeyDown(ByVal keycode)
	...
	If nPlayersInGame > 0 And not bDrainingBalls Then
		...
		If keycode = MechanicalTilt Then
			SoundNudgeCenter
			If GameTime - nTimeMechTilt > MechTiltDebounce Then
...
Sub KickerBooth_Hit
	...
	If bDrainingBalls Then
		...
		bBoothSaved = False
		KickerBooth.TimerEnabled = True
	ElseIf nTimeBallSave > 0 And 1 = BIP Then
		bBoothSaved = True
		fBoothTargetAngle = 190
		KickerBooth.TimerEnabled = True
		nBoothOption = 0
		bBallHeld = True
	ElseIf abBoothReady(nPlayer) And 1 = BIP Then
		ClearEvents
		Set oEvent = new EventItem
...
Sub KickerBooth_Timer
	...
	Else
		bBallHeld = False
		If 1 = BIP And (Not bDrainingBalls) Then
