...
Const myVersion = "TimeLord3.0.10"
...
'	SlayDragonInitiate
	If CalloutActive=False Then CalloutTimer.Enabled=True:CalloutActive=true:PlaySound "CO_AttackTheDragon":End If	
	AddScore 20000000
...
Sub TriggerSuperLoops_Hit()
	...
	If CalloutActive=False Then CalloutTimer.Enabled=True:CalloutActive=true:PlaySound "CO_SuperLoops":End If	
...
Sub OutofControlAward
	...
	Select Case OOCCount
		Case 1:If CalloutActive=False Then CalloutTimer.Enabled=True:CalloutActive=true:PlaySound "CO_SpaceShip":End If	 
			GiOff
		...
		Case 3: If CalloutActive=False Then CalloutTimer.Enabled=True:CalloutActive=true:PlaySound "CO_PowerOfStupidity":End If	
			LS_MajorAward.Play SeqLeftOn,, 1,100
			DMD "", "", "DMD_OOC", eNone, eNone, eNone, 2000, True, ""  
			vpmtimer.addtimer 3000, "DMDScoreNow'"		
			AddScore 3000000			

		Case 4:  If CalloutActive=False Then CalloutTimer.Enabled=True:CalloutActive=true:PlaySound "CO_Fart":End If	
...
Sub SuperJets														'If SuperPlanets complete then light D in Lord & Check Timelord lights
	...
	vpmtimer.addtimer 30000, "StopSuperJets'"
End Sub	
...
Sub ReverseLock4Shot
	...
	If l20.State=2 Then
		...
		If CalloutActive=False Then CalloutTimer.Enabled=True:CalloutActive=true:PlaySound "CO_RaiseLeftRamp":End If	
			AddScore 2000000
	End if

	If l19.State=2 Then 
			l19.State=1:l20.State=2
		If CalloutActive=False Then CalloutTimer.Enabled=True:CalloutActive=true:PlaySound "CO_6thSense":End If	
			AddScore 1000000
	End If
...
Sub KickerReverseLock1_hit()
	If BallCatchInPlay=True Then Reverse1DelayTimer.Enabled=1
...
Sub ReverseLock1Release
	...
	If CalloutActive=False Then CalloutTimer.Enabled=True:CalloutActive=true:PlaySound "CO_YouMustReverseTime":End If
	LowerRampL
	GiOff
	LS_MajorAward.Play SeqBlinking,, 8,85
...
Sub Reverse1DelayTimer_Timer
	ReverseLock1Release
	Reverse1DelayTimer.Enabled=0
End Sub
...
Sub KickerReverseLock2_hit()
	If BallCatchInPlay=True Then Reverse2DelayTimer.Enabled=1
	If BallCatchInPlay=False Then ReverseLock2Release
End Sub

Sub Reverse2DelayTimer_Timer
	ReverseLock2Release
	Reverse2DelayTimer.Enabled=0
End Sub
...
Sub CheckEnableTimeWarpJackpots
	If	TimeLordComplete=False Then: StartTimeWarpJackPots
End Sub	
...
Sub DragonWound1
		StartFlasherAward
		l42.State=1: AddScore 100000000:l4.state=0:l75.State=2 'Set for trigger lair check to enable Time lord to capture the ball
...
Sub DragonWound2
		StartFlasherAward
		l43.State=1: AddScore 100000000:l4.state=0:l75.State=2
	If CalloutActive=False Then CalloutTimer.Enabled=True:CalloutActive=true:PlaySound "CO_DragonWounded2More":End If	
		DMD "", "", "DMD_DragonWounded", eNone, eNone, eNone, 3000, False, ""
		vpmtimer.addtimer 5100, "WatchClosely'"
		vpmtimer.addtimer 3500, "DMDScoreNow'"
End Sub

Sub DragonWound3
		StartFlasherAward
		l44.State=1: AddScore 100000000:l4.state=0:l75.State=2
...
Sub DragonWound4
		StartFlasherAward
		l45.State=1: AddScore 300000000:CheckTimeLordStatusTimer.Enabled=1:l23B.State=0:l4.state=0:l75.State=2:StartPlanetDragon1
...
Sub GameTimer_Timer()
	...
	Next
	TopPlastics.blenddisablelighting = L * 0.5 ' TopPlastics
