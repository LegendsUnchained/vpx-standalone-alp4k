...
'                 VP_Cooks - Buffy The Vampire slayer Script
'                          Version 1.0.2
' ****************************************************************
...
cPuPPack = "VpBuffyVamp"    ' name of the PuP-Pack / PuPVideos folder for this table
...
Sub Table1_Init()
    ...
    With RightMagnet
        .InitMagnet Magnet002, 20
        .GrabCenter = False
        .CreateEvents "RightMagnet"
    End With

    ' Left Lane Magnet Magnet
    Set LeftlaneMagnet = New cvpmMagnet
    With LeftlaneMagnet
        .InitMagnet Magnet004, 20
        .GrabCenter = False
        .CreateEvents "LeftlaneMagnet"
    End With
...
Sub MagnetTimer_timer
    ...
    Light002.State = 1
    vpmTimer.AddTimer 1000, " Light002.State = 0: LeftMagnet.MagnetOn = False '"
    PlaySound "electroiman" 
    DOF 144, DOFPulse
End Sub

     ' Right Magnet timer 
Sub Magnet2Timer_timer
    RightMagnet.MagnetOn = True
    Light004.State = 1
    vpmTimer.AddTimer 1000, " Light004.State = 0: RightMagnet.MagnetOn = False '"
    DOF 145, DOFPulse
End Sub

     ' Left Lane Magnet timer 
Sub LeftlaneMagnetTimer_timer
    LeftlaneMagnet.MagnetOn = True
    Light005.State = 1
    vpmTimer.AddTimer 1000, " Light005.State = 0: LeftlaneMagnet.MagnetOn = False '"
    DOF 146, DOFPulse
End Sub
...
Sub EsferaTimer_Timer
    RotAngle = (RotAngle + 1)MOD 360
  '  Esfera1.RotY = RotAngle
    End Sub
...
Sub IMANAZUL_timer_Timer
    RotAngle2 = (RotAngle2+ 1)MOD 360
    IMANAZUL.RotZ = RotAngle2
    End Sub
...
Sub ChangeSong
    ...
    Else
        Select Case Battle(CurrentPlayer, 0)
            Case 0
                Select Case Balls
            Case 1: PlaySong "m_ball1"
            Case 2: PlaySong "m_ball2"
            Case Else: PlaySong "m_ball3"
        End Select
...
Sub PlayQuote
    If bQuotePLaying Then Exit Sub
    Dim tmp
    tmp = INT(RND * 20) + 1
...
Sub DMDScore()
    ...
    if(dqHead = dqTail)Then
        ...
        Select Case Battle(CurrentPlayer, 0)
            ...
            Case 15:tmp1 = CL("BATTLE THE FIRST " & "Ball " & Balls)
        End Select
        tmp2 = ""
    End If
...
Sub ShowTableInfo
    ...
    Else
        ...
        End If
    End If
    DMD "        VP_COOKS", "        PRESENTS", "d_jppresents", eNone, eNone, eNone, 3000, False, ""
...
Sub GiveRandomAward() 'from the Pyramid
    ...
    DMD CL("THE PORTAL AWARD"), "", "", eNone, eNone, eNone, 50, False, "fx_spinner"
...
Sub RightRampDone_Hit
    ...
    If light31.State = 2 Then
        pupevent 809
        DMD CL("THE SCYTHE"), CL(jackpot(CurrentPlayer)), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_Jackpot"
        AddScore Jackpot(CurrentPlayer)
        LightEffect 2
        FlashEffect 2
    Else
...
Sub StartBattle
    ...
    Select Case NewBattle
        Case 1 'Serpent Yards = Super Spinners
            DMD CL("THE MASTER"), CL("SHOOT THE SPINNERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            SpinCount = 0
        Case 2 'Calakmul = Super Pop Bumpers
            DMD CL("ANGELUS"), CL("HIT THE POP BUMPERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light22.State = 2
            LightSeqBumpers.Play SeqRandom, 10, , 1000
            SuperBumperHits = 0
        Case 3 'Sierra de Chiapas = Ramps
            DMD CL("AMY THE RAT"), CL("SHOOT THE RAMPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits3 = 0
        Case 4 'Karnak = Orbits
            DMD CL("SWEET"), CL("SHOOT THE ORBITS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            OrbitHits = 0
        Case 5 'CopÃ¡n = Shoot the lights 2
            DMD CL("THE MAYOR"), CL("SHOOT THE LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light45.State = 2
            Light46.State = 2
            Light47.State = 2
        ...
        Case 6 'The Pit = Shoot the lights 1
            DMD CL("ADAM"), CL("SHOOT THE LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
        Case 7 'Palenque=  Target Frenzy
            DMD CL("GLORY"), CL("SHOOT THE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqAllTargets.Play SeqRandom, 10, , 1000
            TargetHits7 = 0
        Case 8 'ChichÃ©n ItzÃ¡ = Left & Right Targets
            DMD CL("THE TRIO"), CL("SHOOT THE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light50.State = 2
            TargetHits8 = 0
        Case 9 'Bonampak = Captive Ball
            DMD CL("DRACULA"), CL("SHOOT CAPTIVE BALL"), "", eNone, eNone, eNone, 1500, True, ""
            Light29.State = 2
            CaptiveBallHits = 0
        Case 10 'Monte Alban = Ramps and Orbits
            'uses the ramphits10 to count the hits
            DMD CL("FAITH"), CL("SHOOT RAMPS ORBITS"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits10 = 0
        Case 11 'Uxmal = Blue Targets
            DMD CL("THE GENTLEMEN"), CL("SHOOT BLUE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqBlueTargets.Play SeqRandom, 10, , 1000
            TargetHits11 = 0
        Case 12 'Valley of the Jaguar = Super Loops
            DMD CL("UBERVAMPS"), CL("SHOOT THE LOOPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            Gate2.Open = 1
            Gate3.Open = 1
        ...
        Case 13 'The Grand Cathedral = Follow the Lights 1
            DMD CL("DARK WILLOW RISES"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            FollowTheLights.Enabled = 1
            AddMultiball 2
            StartJackpots
            ChangeGi green
        Case 14 'The City of the Gods = Follow the Lights 2
            DMD CL("THE GIFT"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            FollowTheLights.Enabled = 1
            AddMultiball 2
            StartJackpots
            ChangeGi green
        Case 15 'Mordekai the Summoner - the final battle
            DMD CL("THE FIRST"), CL("SHOOT THE JACKPOTS"), "", eNone, eNone, eNone, 1500, True, ""
            AddMultiball 4
            StartJackpots
            ChangeGi green
    End Select
