...
Dim ScreenType:ScreenType		= 2		' 0=Desktop, 1=2screen		' Script should autodetect this and set correctly
...
Const bAltHadronFlashPosition	= True ' moves flasher up for those whose POV doesnt show the lockbar.
'----- Shadow Options -----
...
Const VolMusic1 = 0.5	' Volume for Cleland Music or Original Gameplay music and Wait Music
Const cVolBGMusic = .4	' Volume for Videos
...
Const bScriptPrintEnabled = False      ' Set True to enable Script.Print/Debug.Print log output

Sub ScriptPrint(ByVal message)
	If bScriptPrintEnabled Then Debug.Print message
End Sub
...
'		MsgBox "Light Color Error:" & light.name & " idx:" & stackIndex & " " & lIndex
		ScriptPrint "Light Color Error:" & light.name & " idx:" & stackIndex & " " & lIndex
	End If 
End Sub
...
ScriptPrint "Start Service:" & bInService
		...
		elseif serviceLevel<>kMenuNone then 
			if bServiceEdit = False then 
				...
				elseif keycode=ServiceDownKey then 	' Right 
					...
					direction=1
				End if
ScriptPrint "EDIT DT:" & dataType & " min:" & minVal & " Max:" & maxVal & " dir:" & direction
				if direction<>0 then 
					if dataType="INT" or dataType="PCT" then 
						if DMDMenu(serviceIdx).StdIdx <> -1 then 
					...
					elseif dataType="BOL" then 
						if DMDMenu(serviceIdx).StdIdx <> -1 then 
ScriptPrint "EDIT BOOL:" & DMDStd(DMDMenu(serviceIdx).StdIdx)&""
...
ScriptPrint "Skip Command >"&FlipperSkipCmd&"<"
...
ScriptPrint "Start Instant " & keycode & " " & bInstantInfo
...
Sub InstantInfoTimer_Timer
    InstantInfoTimer.Enabled = False
    bInstantInfo = True
ScriptPrint "Instant Info timer"
	PuPlayer.LabelShowPage pOverVid, 1,0,""
...
ScriptPrint "EndInstantInfo check" & keycode & " " & InstantInfoTimer.UserValue
		if (keycode=InstantInfoTimer.UserValue) then 	' They let go of the key
ScriptPrint "EndInstantInfo"
			ResetBallSearch
			InstantInfoTimer.Enabled = False
			bInstantInfo=False
...
ScriptPrint "lNovaStatesKyln(0)" & PlayerState(CurrentPlayer).lNovaStatesKyln(0) & " " & PlayerState(CurrentPlayer).lNovaStatesKyln(1)
		...
		if bSkillshotsReady(0) or bSkillshotsReady(1) then 
			...
			End If
ScriptPrint "bSkillshotsReady1: " & bSkillshotsReady(0) & " " & bSkillshotsReady(0) & " Nova " & lNova_sw8.UserValue & " " & lNova_sw9.UserValue
...
'Handle Skillshot 
	elseif bSkillshotsReady(0) or bSkillshotsReady(1) then 
		...
		Else
			...
			lNova_sw9.UserValue = 2
		End If
ScriptPrint "bSkillshotsReady2: " & bSkillshotsReady(0) & " " & bSkillshotsReady(0) & " Nova " & lNova_sw8.UserValue & " " & lNova_sw9.UserValue
		SetLightColor lNova_sw8, "orange", lNova_sw8.UserValue
		SetLightColor lNova_sw9, "orange", lNova_sw9.UserValue
...
' Normal Handleing 
		...
		End If
ScriptPrint "bSkillshotsReady3: " & bSkillshotsReady(0) & "Nova " & PlayerState(CurrentPlayer).lNovaStates(3) & " " & PlayerState(CurrentPlayer).lNovaStates(4)
...
Function getQueueTime()		' Returns how much time left on queue
	Dim time,i 
	time = 0
ScriptPrint "GetQueueTime:" & now 
ScriptPrint "GetQueueTime:" & QueueCurrentTime & " " & QueueActive
	if QueueActive and QueueCurrentTime <> 0 then time = (DateDiff("s", now, QueueCurrentTime) * 1000)
ScriptPrint "GetQueueTime Active:" & time

	for i = 0 to PupQueueEndPos
		time = time + PupQueue(i, 2) 
	Next
	getQueueTime = time
ScriptPrint "GetQueueTime ret:" & time
End Function
Sub QueuePop()
...
ScriptPrint "--Q-Dump Pop---"
	Dim xx
	for xx = 0 to PupQueueEndPos
		ScriptPrint xx & " " & PupQueue(xx, 0) & " " & PupQueue(xx, 1) & " " & PupQueue(xx, 2) & " " & PupQueue(xx, 3) 
	Next 
ScriptPrint "--Q-Dump Pop---"
...
ScriptPrint "Queue Scene " & Command & " Len: " & msecLen
...
Sub RunQueue(bNewItem)
	dim qCmd, qTime
ScriptPrint "Run Queue " & QueueActive & " " & bNewItem & " " & Now
...
ScriptPrint QueueCurrentTime
			...
			End If
		Else 
ScriptPrint "Queue Empty Deacivated"
			QueueActive = False
		End If 
	End if
End Sub
	...
	if Index <> -1 then 	' We are adding the standard bonus 
		Select case index
			...
			case "8"  ' StarLord
				BonusArrowHits(index) = BonusArrowHits(index) + 5000
		End Select
ScriptPrint "BonusAddModeHit " & index &  " " & BonusArrowHits(index)
...
ScriptPrint "ModePoints: " & ModePoints
'			For i = mode to 8
...
'			Next 
		...
		case 1	' Scroll through Hit Bonus (1.x)
			...
			For i = mode to 8
ScriptPrint "BonusIdx: " & i & " " & BonusArrowHits(i)
...
if tmpModeIndex <> -1 then ScriptPrint "BL: " & i & " - HITS: " & BonusArrowHits(i) & " ModeBonus: " & BonusModeTotal(i) & " TotalBonus:" & ModeBonus &  " ModePercent: " & ModePercent(tmpModeIndex) & " " & tmpModeIndex
...
Sub EndOfBall2()
    ...
    Else ' no extra balls
        ...
        If(BallsRemaining(CurrentPlayer) <= 0) Then				' GAME OVER
            ScriptPrint "No More Balls, High Score Entry"
			if bUsePupDMD then 
				PuPlayer.PlayStop pOverVid						' Stop overlay if there is one
				PuPlayer.SetLoop pOverVid, 0
			End If
			...
			playclear pAudio
			'StopSound "m_wait"
ScriptPrint "Stopping sound m_wait"
			bSkipWaitVideo=True
			StopPlayerMode
			StopPlayerMode2
			bSkipWaitVideo=False
...
Sub EndOfBallComplete()
    ...
    ScriptPrint "EndOfBall - Complete "
    ...
    ScriptPrint "Next Player = " & NextPlayer
...
Sub DrainDummy_Hit()	' Call this to short cycle outlane drains 
	ScriptPrint "DrainDummy_Hit:" & BallsOutlaneDrainCnt
...
ScriptPrint "Ball Save multiball"
            if bShortCycle then 
				AddMultiballFast 1
			Else 
				AddMultiball 1
			...
			DoBallSave
        Else
ScriptPrint "Drain " & BallsOnPlayfield & " " & RealBallsInLock & " " & bMultiBallMode
...
Sub swPlungerRest_Hit()
    ScriptPrint "ball in plunger lane " & bPlayerModeSelect & " " & bAutoPlunger & " " & bCreatedBall & " " & PlayerMode
...
ScriptPrint "Ballsaver Ready:" &  bBallSaverReady & " T:" & BallSaverTime & " Active:" & bBallSaverActive
...
Sub EnableBallSaverForce(seconds)
    ScriptPrint "Ballsaver started " & seconds
...
ScriptPrint "Add Score: " & points & " x" & multiplierVal
...
Sub HadronFlash(color, enabled)
	if enabled Then 
ScriptPrint "Hadron Flash: " & color
...
Sub tmrBallSearch_Timer()	' We timed out
	...
	if bGameInPlay and bPlayerModeSelect = False and _
		...
		ScriptPrint "Ball Search - NO ACTIVITY " & BallSearchCnt

		dim Ball
		ScriptPrint "--- listing balls ---"
...
Sub HurryUpFinish()
ScriptPrint "HurryUpFinish:" & bQuillsQuestBonusWait
...
ScriptPrint "DEBUG L:" & bDebugLeftMagnaDown & " R:" & bDebugRightMagnaDown 
	...
	elseif (bDebugLeftMagnaDown and bDebugRightMagnaDown) then 	' Setup for Wizard modes 
		if modesCompleted < 4 and bCherryBombDone=False then ' Cherry Bomb
			ScriptPrint "Enable Cherry Bomb"
			ModePercent(0) = 100
			ModePercent(1) = 100
			ModePercent(2) = 100
			ModePercent(3) = 100
...
ScriptPrint "UpdatePlayerMode " & PlayerMode 
    LightSeqSkillshot.Play SeqAllOff
    'turn off lights
...
ScriptPrint "Immolation:" & bValidHit & " " &  bImmolationSJPReady

		if bValidHit and bImmolationSJPReady then ' Hit SJP
ScriptPrint "Immolation Complete"
		...
		elseif bValidHit Then
			...
			Else 
				...
				For i = 0 to 8
					if bImmolationProgress(i)=False then 
						bValidHit=True
ScriptPrint "Immolation arrow not complete:" & i
					End If 
				Next
...
ScriptPrint "Xandar:" & trigger_name
...
ScriptPrint "Finished: " & hitLight.name & " " & hitLight.UserValue 
			End If

			bFinalShot=True		' See if we got all of the arrows 
			For each a in aRampLights
				if a.UserValue < 3 Then
ScriptPrint "Not Finished: " & a.name & " " & a.UserValue & " " & StackState(kStack_Pri2).GetArrowState(a)
					bFinalShot=False
					exit For
				End If
			Next
...
ScriptPrint "Debounce"
		exit Sub
	End If
ScriptPrint "Check Mode Progress " & trigger_name & " " & BallSearchCnt 
...
ScriptPrint "GrootMBJackpotCnt: " & GrootMBJackpotCnt & " " & GrootSJPReady
		For each a in aRampLights		' Need to hit flashing arrows and turn them solid
...
ScriptPrint "GrootMBJackpotHits(CurrentPlayer, i): " & GrootMBJackpotHits(CurrentPlayer, i)
							SSetLightColor kStack_Pri1, b, "green", 2
						End if 
					Next
ScriptPrint "bModeComplete: " & bModeComplete
...
ScriptPrint "StackState(kStack_Pri1).GetArrowState(a):" & StackState(kStack_Pri1).GetArrowState(a) & " " & GrootMBJackpotHits(CurrentPlayer, i)
...
ScriptPrint "Start GROOT SUPER JP"
					StartGrootFinalShot
				End if 
...
ScriptPrint "ORB: " & OrbMBJackpotHits & " " & trigger_name
...
'						End If
				...
				Else
					...
					i = GetModeIndex(trigger_name)
					OrbMBJackpotHits = OrbMBJackpotHits + 1
ScriptPrint "Orb Jackpot for " & trigger_name & ":" & OrbMBJackpotHits
...
ScriptPrint "Check progress 2nd: " & bSecondMode & " " & PlayerMode & " " & trigger_name
	if (bSecondMode) Then
		...
		if PlayerMode <> -1 Then
			ScriptPrint "Mode Progress: " & ModePercent(PlayerMode)
			if ModePercent(PlayerMode) >= 100 then 
				ScriptPrint "Skip Score: " & ModePercent(PlayerMode)
				Exit Sub 
			End If 
		End If 
        ...
        Case 2: ' Yondu - Yaka Arrow (8 shots)
			...
			if trigger_name = "lStarLordArrow" and bModeProgressUpgraded=False Then	' scoop hit - perform upgrade
				...
				if (bModeProgressUpgraded) and (trigger_name = "tgHadron0" or trigger_name="tgHadron1") then 	' Upgrade allows left hadrons to spot shots
					...
					For each a in aRampLights
if a.name = trigger_name then ScriptPrint "Yondu Arrow: " & trigger_name & " State:" & StackState(kStack_Pri0).GetArrowState(a)
...
'					vpmtimer.addtimer 3000, "QuillsQuestBonus '"		' Start timer to hit scoop again and get bonus
				...
				For each a in aRampLights
if a.name = trigger_name then ScriptPrint "Arrow State: " & StackState(kStack_Pri0).GetArrowState(a)
					if a.name = trigger_name and StackState(kStack_Pri0).GetArrowState(a) <> 0 Then		' Hit Ramp that was lit
						bDebounce = True						' so the ball doesnt roll over twice before we reset
						vpmtimer.addtimer 400, "debounceReset '"
						'PlaySound "gg_RampHit"
...
ScriptPrint "StartQQFinalHurryup:"
	HurryUpFinish
...
ScriptPrint "CollectQQFinalHurryUp:" & bQuillsQuestBonusWait
	bQuillsQuestBonusWait=False
...
Sub StartPlayerModeVideo(bSkipInitial)	' Play the GamePlay Video - SkipInitial skips the scene where we show the splash for this mode
	...
	dim newVolume
	newVolume=1
ScriptPrint "StartPlayerModeVideo: " & bSecondMode & " " & PlayerMode
	'StopSound "m_wait"
...
Sub StopPlayerModeVideo()	' chooses the wait video 
	...
	if bGameInPlay = false then exit sub  
ScriptPrint "StopPlayerModeVideo: (Mix Music/video)"
...
Sub StartPlayerMode()
ScriptPrint "StartPlayerMode " & PlayerMode
	pDMDSetPage(pScores)
	setUpgradeLight(True)
	setModeSelectLight(False)
'	flshUpgrade.TimerEnabled = True
...
Function Bonus2ModeIndex(BonusIndex)
	...
	End Select
ScriptPrint "BonusIndex: " & BonusIndex & " ret:" & Bonus2ModeIndex
end Function
...
Sub StopPlayerMode()
ScriptPrint "StopPlayerMode " & PlayerMode
	bIModeComplete=False
	ModeCountdownTimer.Enabled = False
	setUpgradeLight(False)
'	flshUpgrade.TimerEnabled = False
...
ScriptPrint "2nd Mode? " & ModePercent(PlayerMode) & " 2ndMode:" & bSecondMode
...
ScriptPrint "SetModeLightComplete: " & idx & " pct:" & ModePercent(Idx)
...
Sub CheckModeProgress2nd(trigger_name)
	...
	if PlayerMode <> -1 Then		' Just in case we get back in here we bail because we are already 100% progress
		ScriptPrint "Mode Progress2: " & ModePercent(PlayerMode)
		if Mode2Percent(PlayerMode) >= 100 then 
			ScriptPrint "WE SHOULDNT GET HERE.  Skip Score: " & ModePercent(PlayerMode)
			Exit Sub 
		End If 
	End If
...
ScriptPrint "Broker Switch:" &  SwitchHitCount
				if (SwitchHitCount <= 0) then
					bModeComplete = True
					Mode2Percent(PlayerMode2) = 100
				End If
...
ScriptPrint "StartPlayerMode2:" & PlayerMode2
...
Sub StopPlayerMode2()
	If bSecondMode then
ScriptPrint "StopPlayerMode2 " & PlayerMode
		Dim a
		For each a in aRampLights				' Turn off all the ramp lights to reset the mode
			SSetLightColor kStack_Pri0, a, "white", 0
		Next
...
Sub CherryBombDrained
	...
	if BallsOnPlayfield<>0 then 		' Make sure all the balls are done
		...
		Exit Sub 
	End if 
ScriptPrint "CherryBombDrained: " & BallsOnPlayfield & " " & ModeCountdownTimer.UserValue
...
ScriptPrint "COMBO! " & MultiplierShot & " " & ComboCount
	...
	if bLeftSide then 
ScriptPrint "COMBO LEFT:" & MultiplierShot
		...
		SetLightColorRestore lNebulaArrow, -1 
	Else 
ScriptPrint "COMBO Right:" & MultiplierShot
...
ScriptPrint "COMBO END:" & tmrCombo.UserValue
...
Sub CheckShotMultiplier
	...
	if bShotMultiplierSelect then ScriptPrint "Already in 2x Multiplier"		' We can do this for each 
...
Sub HandleSkillshot()
ScriptPrint "HandleSkillshot:" & bSkillshotsReady(0) & " " & bSkillshotsReady(1)
...
ScriptPrint "Scoop Hit: PlayerMode:" & PlayerMode & " MB:" & bMultiBallMode
	if tmrCherryBomb.Enabled Then	' Go into cherry bomb mode
		CherrySong
ScriptPrint "Scoop Cherry"

	elseif tmrImmolation.Enabled then 
		' Do nothing
ScriptPrint "Scoop Immolation"

	elseif tmrXandar.Enabled then
		' Do nothing
ScriptPrint "Scoop Xandar: " & bSaveXandarReady & " " & bXandarReady
		if bSaveXandarReady and bXandarReady=False then Exit Sub		' We just started dont kick
	elseif bSecondMode then
		' Do nothing
ScriptPrint "Scoop 2nd Mode"
...
ScriptPrint "Must Finish OrbMB or GrootMB"
		else 
			QueueFlush()
...
ScriptPrint "FakeHadronHit:" & LightName
...
ScriptPrint "CheckHadron:" & HadronEnforcerCount
...
ScriptPrint "HadronCheck cnt:" & HadronEnforcerCount & " used:" & bUsed & " useIt:" & bUseIt
...
Sub OrbHurryUpFinish()
	ScriptPrint "OrbHurryUpFinish:" & OrbMBJackpotHits
	dim a
...
ScriptPrint "OrbTarget1_Dropped " & bOrbRealHit
	' Check Broker Arrow
	if bOrbRealHit then tBroker_m_Hit		' Only real hits trigger score 
...
Dim i
Dim value
ScriptPrint "Groot MB Timer " & tmrGrootMBBonus.UserValue
...
ScriptPrint "Groot MB Release " & BallsInLock
...
Sub StartGrootMB() 'Multiball
	...
	ScriptPrint "StartGrootMB: Remaining:" & shotsRemaining & "/" & 18-GrootMBJackpotCnt
...
Function EndGrootMB(checkRelease)
	EndGrootMB=False
	if bGrootMultiball then 
ScriptPrint "check Release: " & checkRelease & " pfBalls:" & BallsOnPlayfield & " lock:" &  BallsInLock & " GrootMBBonus:" & tmrGrootMBBonus.Enabled
...
ScriptPrint "pause media ch:" & channel & " current:" & bMediaPaused(channel) & " " & lastPlayMS(channel)
...
ScriptPrint "Pausing with delay"
	...
	sub resumemedia(channel) 
		if bUsePUPDMD=False then exit sub 
ScriptPrint "resume media ch:" & channel & " current:" & bMediaPaused(channel)
		if bMediaSet(channel) = False then Exit Sub 
...
ScriptPrint "PlayClearDelayed:" & lastPlayMS(chan)
		PlayClearDelayed = (lastPlayMS(chan) < 400)
	End Function 

	sub playclear(chan)
		if bUsePUPDMD=False then exit sub 
		ScriptPrint "play clear'd " & chan
...
ScriptPrint "Stopping with delay"
...
ScriptPrint "Stopping with delay"
		...
		if chan = pDMDText or chan = pDMDFull Then
			if currentqueue <> "" then 
				bCancelNext = True
ScriptPrint "Clear is cancelling " & currentqueue
			End If 
			PuPlayer.SetBackGround chan, 0
			PuPlayer.SetLoop chan, 0
			PuPlayer.playstop chan
...
ScriptPrint "RESET:" & lastPlayMS(channel) & " " & channel
...
ScriptPrint "ChangeVol pMusic for Cineon"
...
ScriptPrint "PlayMedia " & channel & " Dir:" & playlist & " File:" & name & " Vol:" & audiolevel & " Pri:" & priority & " Len:" & length
...
ScriptPrint "ChangeVol pDMDText Volume for Callouts"
...
ScriptPrint "ChangeVol pDMDText for OverVid" 
...
ScriptPrint "Change Volume5"
		...
		ScriptPrint "turnitbackup "
		'puplayer.setvolume pMusic sndtrkvol
	End Sub

	Sub turnitbackupvid
ScriptPrint "Change Volume4"

		PuPlayer.SendMSG "{ ""mt"":301, ""SN"": "&pDMDText&", ""FN"":11, ""VL"":"&VolBGMusic&" }"
		ScriptPrint "turnitbackupvid "
	End Sub

	Sub turnitbackupcine
ScriptPrint "Change Volume3"
		PuPlayer.SendMSG "{ ""mt"":301, ""SN"": "&pMusic&", ""FN"":11, ""VL"":"&VolMusic&" }"
		ScriptPrint "turnitbackupcine "
	End Sub
...
ScriptPrint "Cancel Queue"
			bCancelNext=False
			Exit Sub
		End If 
ScriptPrint "Executing " & currentqueue
		Execute currentqueue
		currentqueue = ""
		bCancelNext = False 	' Items in queue can trigger a cancel
ScriptPrint "Queue Empty"
'		if skipped = 0 Then
'			noskipper=0
...
Class Dampener
	...
	public sub Dampen(aBall)
		...
		if Print then ScriptPrint Round(cor.ballvel(aBall.id),2) & ", " & round(desiredcor,3)
...
Sub Scorbit_LOGUpload(state)	' Callback during the log creation process.  0=Creating Log, 1=Uploading Log, 2=Done 
	Select Case state 
		case 0:
			ScriptPrint "CREATING LOG"
		case 1:
			ScriptPrint "Uploading LOG"
		case 2:
			ScriptPrint "LOG Complete"
	End Select 
End Sub 
'<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
'		bProduction=0
		...
		for each Nad in Nads
			if not isnull(Nad.MACAddress) then
				ScriptPrint "Using MAC Addresses:" & Nad.MACAddress & " From Adapter:" & Nad.description   
...
'			Debug.print "CALLBACK: " & objXmlHttpMain.Status & " " & objXmlHttpMain.readystate
			if objXmlHttpMain.Status=200 and objXmlHttpMain.readystate = 4 then 
				...
				ScriptPrint "RESPONSE: " & ResponseStr
...
'Debug.print "Parts2:" & Parts2(i)
	...
	Public Sub StartSession()
		if bEnabled=False then Exit Sub 
ScriptPrint "Scorbit Start Session" 
		CachedPlayerNames(0)=""
		CachedPlayerNames(1)=""
		CachedPlayerNames(2)=""
		CachedPlayerNames(3)=""
...
ScriptPrint "Scorbit Stop Session" 
...
'		SendHeartbeat
		...
		if bUploadLog and LogIdx<>0 and bCancel=False then 
			ScriptPrint "Creating Scorbit Log: Size" & LogIdx
			Scorbit_LOGUpload(0)
...
'					"&session_sequence=" & SessionSeq & "&active=" & bActive
		...
		if resultStr<>"" then ScriptPrint "SendUpdate Resp:" & resultStr
	End Sub 
...
'		End if
	...
	Private Sub SendHeartbeat()
		...
		ScriptPrint "Heartbeat Resp:" & resultStr
...
ScriptPrint "RUNNING Command:" & Command
		rc = wsh.Run(Command, windowStyle, waitOnReturn)
ScriptPrint "Return:" & rc
...
ScriptPrint "Got TOKEN:" & sToken
				getStoken=True
			Else 
ScriptPrint "ERROR:" & result
				getStoken=False
			End if 
		else 
ScriptPrint "ERROR No File:" & rc
		End if 
...
ScriptPrint "Url:" & Url  & "  Async=" & bRunAsynch
...
'		on error resume next
			...
			elseif err.number <> 0 then 
				ScriptPrint "Server error: (" & err.number & ") " & Err.Description
			End if 
			if bRunAsynch=False then 
ScriptPrint "Status: " & objXmlHttpMain.status
...
ScriptPrint "PostMSg:" & Url & " " & PostData
...
ScriptPrint "Posting File " & sUrl & " " & sFileName & " " & bAsync & " File:" & Mid(sFileName, InStrRev(sFileName, "\") + 1)
...
'Debug.print "POSTDATA: " & sPostData & vbcrlf
		...
		With objXmlHttpMain
			...
			If Not bAsync Then
				...
				pvPostFile = Response
ScriptPrint "Upload Response: " & Response
			End If
		End With
...
' alltime scores
	...
	for each x in alltime
		...
		ScriptPrint "alltimevar(" &MyNum & ")=" & x
	Next
...
Sub pDMDSetPage(pagenum)
	If (bUsePUPDMD) then
		ScriptPrint "Changing Page: " & pagenum
...
DIM curPos
	if pDMDCurPriority>pPriority then 
		...
		ScriptPrint "DMD Skipping - Hi Pri"
	End If 
...
Sub pBgShowBonus(Message1, Message2)
	if bUsePUPDMD then
ScriptPrint "Showing Bonus: " & Message1 & " - " & Message2
...
ScriptPrint "ATTRACT Next:" & pCurAttractPos & " " & bGameInPlayHidden & " " & bGameInPlay
