...
' 1.2.1 - bthlonewolf/Wizball - fix swordmask
'
...
Sub DMDSettingsInit()
    ...
    DMDMenu(9).Setup "REPLAY  TYPE", 						kDMDStd_ReplayType,	    2,		"LST[1,EXTRA GAME:2,EXTRA BALL]", 1 
    ...
    DMDMenu(12).Setup "FREE  PLAY",                          kDMDStd_FreePlay,       1,     "BOOL", 1
...
Sub ResetForNewPlayerBall()
    ...
    CurrentWildfire = 0
    SwordsCollected = 0
	SwordMask = 0 ' 1.2.1 fix for swordmask
...
sub PrintSwordMask
	...
	do
		s=s & "|" & (SwordMask And (2^x))
		x=x+1
	loop while x<8

	debug.print "Swordmask; " & s
	debug.print "SwordsCollected: " & SwordsCollected
end sub


Sub DoAwardSword
	'modified for swordmask issue to make more resilient
    Dim scene,font,i,j
	Dim availableSwords(7), swordCount
	...
	swordCount = 0
	For i = 0 To 7
		If (SwordMask And (2^i)) = 0 Then
			availableSwords(swordCount) = i
			swordCount = swordCount + 1
		End If
	Next
	If swordCount = 0 Then Exit Sub
	i = availableSwords(RndNbr(swordCount) - 1)
