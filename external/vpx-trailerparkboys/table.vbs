...
'        Trailer Park Boys - Binballers Pinball Script JPSalas & marty
'                         Table by Clairvius
'							Version 1.0
' ****************************************************************
...
Const cGameName = "Trailer Park Boys Binballers"
Const myVersion = "1.0"
...
Sub DMDScore()
    ...
    if(dqHead = dqTail)Then
        ...
        Select Case Battle(CurrentPlayer, 0)
            ...
            Case 15:tmp1 = CL("ARK MISSION " & "Ball " & Balls)
        End Select
        tmp2 = ""
    End If
...
Sub ShowTableInfo
    ...
    Else
        ...
        End If
    End If
    DMD "          CLAIRVIUS", "           PRESENTS", "d_jppresents", eNone, eNone, eNone, 3000, False, ""
    DMD "", "", "d_title", eNone, eNone, eNone, 4000, False, ""
    DMD "", CL("SMOKE WEED EVERY DAY "), "", eNone, eNone, eNone, 2000, False, ""
...
Sub LeftSlingShotht_Slingshot
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF("fx_slingshot", 103, DOFPulse, DOFcontactors), Lemkht
    DOF 105, DOFPulse
    LeftSling4ht.Visible = 1
    Lemkht.RotX = 26
    LStepht = 0
    Leftslingshotht.TimerEnabled = True
    ' add some points
    AddScore 210
    ' add some effect to the table?
    ' remember last trigger hit by the ball
...
Sub LeftSlingShotht_Timer
    Select Case LStepht
        Case 1:LeftSling4ht.Visible = 0:LeftSLing3ht.Visible = 1:Lemkht.RotX = 14
        Case 2:LeftSling3ht.Visible = 0:LeftSling2ht.Visible = 1:Lemkht.RotX = 2
        Case 3:LeftSLing2ht.Visible = 0:Lemkht.RotX = -10:Leftslingshotht.TimerEnabled = 0
    End Select
    LStepht = LStepht + 1
End Sub
...
Sub Check5BankTargets
    ...
    If light15.state + light16.state + light17.state + light18.state + light19.state = 5 Then
        ...
        DMD "YOU HAVE FOUND", "SOME DOPE", "monster_" &tmp, eNone, eNone, eNone, 2500, True, ""
        LightEffect 1
        FlashEffect 1
...
Sub GiveRandomAward() 'from the Pyramid
    ...
    DMD CL("HASH AWARD"), "", "", eNone, eNone, eNone, 50, False, "fx_spinner"
...
Sub RightRampDone_Hit
    ...
    If light31.State = 2 Then
        DMD CL("INDY GUN"), CL(jackpot(CurrentPlayer)), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_Jackpot"
        AddScore Jackpot(CurrentPlayer)
        LightEffect 2
        FlashEffect 2
    Else
...
Sub StartBattle
    ...
    Select Case NewBattle
        Case 1 'Serpent Yards = Super Spinners
            DMD CL("RANDY HAS YOUR WEED"), CL("SHOOT THE SPINNERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            SpinCount = 0
        Case 2 'Calakmul = Super Pop Bumpers
            DMD CL("LAHYE HAS YOUR WEED"), CL("HIT THE POP BUMPERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light22.State = 2
            LightSeqBumpers.Play SeqRandom, 10, , 1000
            SuperBumperHits = 0
        Case 3 'Sierra de Chiapas = Ramps
            DMD CL("MAKE SOME HASH"), CL("SHOOT THE RAMPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits3 = 0
        Case 4 'Karnak = Orbits
            DMD CL("THE BIG DIRTY"), CL("SHOOT THE ORBITS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            Light45a.State = 2
            OrbitHits = 0
        Case 5 'CopÃ¡n = Shoot the lights 2
            DMD CL("MAKE HOMEBREW"), CL("SHOOT THE LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light45.State = 2
            Light46.State = 2
            Light47.State = 2
        ...
        Case 6 'The Pit = Shoot the lights 1
            DMD CL("HARVEST WEED"), CL("SHOOT THE LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
        Case 7 'Palenque=  Target Frenzy
            DMD CL("GET SMOKES"), CL("SHOOT THE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqAllTargets.Play SeqRandom, 10, , 1000
            TargetHits7 = 0
        Case 8 'ChichÃ©n ItzÃ¡ = Left & Right Targets
            DMD CL("FIND CHANGE"), CL("SHOOT THE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light50.State = 2
            TargetHits8 = 0
        Case 9 'Bonampak = Captive Ball
            DMD CL("B AND E"), CL("SHOOT CAPTIVE BALL"), "", eNone, eNone, eNone, 1500, True, ""
            Light29.State = 2
            CaptiveBallHits = 0
        Case 10 'Monte Alban = Ramps and Orbits
            'uses the ramphits10 to count the hits
            DMD CL("SHOOT A PORNO"), CL("SHOOT RAMPS ORBITS"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits10 = 0
        Case 11 'Uxmal = Blue Targets
            DMD CL("FIND RICKY"), CL("SHOOT BLUE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqBlueTargets.Play SeqRandom, 10, , 1000
            TargetHits11 = 0
        Case 12 'Valley of the Jaguar = Super Loops
            DMD CL("FIND BUBBLES"), CL("SHOOT THE LOOPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            Gate2.Open = 1
            Gate3.Open = 1
        ...
        Case 13 'The Grand Cathedral = Follow the Lights 1
            DMD CL("ESCAPE FROM LAHEY"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            FollowTheLights.Enabled = 1
            AddMultiball 2
            StartJackpots
            ChangeGi green
        Case 14 'The City of the Gods = Follow the Lights 2
            DMD CL("PAY LOT FEES"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            FollowTheLights.Enabled = 1
            AddMultiball 2
            StartJackpots
            ChangeGi green
        Case 15 'Mordekai the Summoner - the final battle
            DMD CL("SELL HASH"), CL("SHOOT THE JACKPOTS"), "", eNone, eNone, eNone, 1500, True, ""
            AddMultiball 4
            StartJackpots
            ChangeGi green
    End Select
