...
' ***************************************************************************************************************
'
'                                 BlOOD EDITION - Mod by Arminio, v1.0 2025
'                                Made from BALUTITO mod v3 - with permission
'
'                                      Details about original are below
'      Thanks BALUTITO for permission to mod his version and thanks to all auhors of original table and mods
' ---------------------------------------------------------------------------------------------------------------
' Changes:
'    - New colors of various lights, ramps, moon, various graphics elements... to get this blood red atmosphere
'    - New lights and shades added (ramps, Dracula's eyes, apron, desktop mode background...)
'    - Fixes: transparency bug on apron (ball was visible through apron when drained), small moonlight leakage
'    - Added some new features like red ball, start/stop music (using magna save keys), various behavior
'      of moonlight, red eyes, red moon etc. - all this can be configured with options - check section "BLOOD
'      EDITION OPTIONS" below
'    - Added some ASCII art at the top of the script (Well, why not! :) )
'    - Reduced VPX size below 100MB (using lossless compressed webp images)
'
' ***************************************************************************************************************
'
'
'
'
' Mod By BALUTITO 2021 thank's to JPSalas and Thalamus
...
'-----------------------
' BLOOD EDITION OPTIONS
'-----------------------
Const StartMusic = True  			  'True = MP3 music track will automatically start when table is started, False = no music at start
Const MagnaSaveMusicStartStop = False  'True = enables manual start/stop of MP3 music vith Magna Save keys (right - start music, left - stop music)
Const MoonLight = True               'True = every ball launch will turn moonlight on, False = original behaviour
Const DraculaEyesLightsBlink = True   'True = Dracula's red glowing eyes will slowly (creepy :) ) fade in/out, False = eyes glowing constantly
Const RedBall = True                  'True = play with red ball, False = play with normal ball
Const RedMoon = True                'True = red moonlight, False = "standard" moonlight
'------------------------------
' END OF BLOOD EDITION OPTIONS
'------------------------------
...
Sub Table1_Init
	...
	If RedMoon = True Then
		Primitive232.image ="dome2_0_red_dark"
		l58.colorfull = rgb(255, 50, 50)
		l58.color= rgb(255, 155, 155)
		l58a.colorfull = rgb(255, 0, 0)
		l58a.color= rgb(255, 0, 0)
	End If


	If RedBall = True Then 
		Table1.BallImage="JPBall-Dark2-highred"
	Else
		Table1.BallImage="JPBall-Dark2"
	End If


	If DraculaEyesLightsBlink= True Then
		Eye001.state = 2
		Eye002.state = 2
	Else
		Eye001.state = 1
...
music = "drac.mp3"
if  StartMusic=True Then PlayMusic "drac.mp3" 
...
Sub table1_KeyDown(ByVal Keycode)
	...
	If MagnaSaveMusicStartStop = true Then
		If keycode = LeftMagnaSave Then EndMusic
		If keycode = RightMagnaSave Then PlayMusic "drac.mp3"
	End If
