...
Const cGameName = "Old_Chicago_1975"
...
Const HSFileName="OldChicago_75VPX.txt"
Const B2STableName="Old_Chicago_1975"
...
Sub Table1_Init()
	...
	if BallsPerGame=0 then BallsPerGame=5
	...
	if DoubleBonusOption=0 then
			tempimg=tempimg+"0"
	else
			tempimg=tempimg+"1"
	end if
	InstructCard.image=tempimg
...
'	PlaySound "Plunger"
	...
	EMReel12.ResetToZero
	EMReel11.ResetToZero
	for i=1 to 4
    player=i
		If B2SOn Then 
...
Sub Table1_KeyDown(ByVal keycode)
	...
	If keycode=LeftTiltKey and InProgress=true Then
		Nudge 90, 2
		TiltIt
	End If
    
	If keycode=RightTiltKey and InProgress=true Then
		Nudge 270, 2
		TiltIt
	End If
    
	If keycode=CenterTiltKey and InProgress=true Then
		Nudge 0, 2
		TiltIt
	End If
   ...
   if keycode = StartGameKey and Credits>0 and InProgress=true and Players>0 and Players<4 and BallInPlay<2 then
		Credits=Credits-1
		if Credits = 0 Then DOF 123, 0
		EMReel10.SetValue(Credits)
		Players=Players+1
		EMReel11.ResetToZero
		EMReel11.SetValue(Players)
		playsound "click"
		If B2SOn Then
...
'END GNMOD
		...
		Bumper1Light.state=0
		TopRolloverLightLeft.state=0
		Bumper2Light.state=1
		BumpersOn
		TopRolloverLightRight.state=1
		for each obj in SpinLights
			obj.state=0
		next
		SpinLights(0).state=1
		SpinnerCounter=0
		BonusMultiplier=1
		If B2SOn Then
			Controller.B2SSetTilt 0
...
'***************************************** Drop Targets ***************************************************
Sub DropTarget1_Hit()
	If (DropTarget1.IsDropped = False) and (TableTilted=false) then
...
Sub DropTarget2_Hit()
	If (DropTarget2.IsDropped = False) and (TableTilted=false) then
		PlaySound"droptargetdropped"
		DOF 118,2
		SetMotor(500)
		TargetsDownCounter=TargetsDownCounter+1
...
Sub DropTarget3_Hit()
	If (DropTarget3.IsDropped = False) and (TableTilted=false) then
		PlaySound"droptargetdropped"
		DOF 118,2
		SetMotor(500)
		TargetsDownCounter=TargetsDownCounter+1
...
Sub DropTarget4_Hit()
	If (DropTarget4.IsDropped = False) and (TableTilted=false) then
		PlaySound"droptargetdropped"
		DOF 118,2
		SetMotor(500)
		TargetsDownCounter=TargetsDownCounter+1
...
Sub DropTarget5_Hit()
	If (DropTarget5.IsDropped = False) and (TableTilted=false) then
		PlaySound"droptargetdropped"
		DOF 118,2
		SetMotor(500)
		TargetsDownCounter=TargetsDownCounter+1
...
'********************************* Rubber triggers ***********************************************************
Sub RubberScoreWall8_Hit()
	If TableTilted=false then
		AddScore(10)
	End If
End Sub

Sub RubberScoreWall9_Hit()
	If TableTilted=false then
		AddScore(10)
	End If
End Sub
...
Sub TriggerLeft500_Hit()
	If TableTilted=false then
		PlaySound "sensor"
		DOF 113,2
		if TopRolloverLightLeft.state=1 then
			SetMotor(3000)
			IncreaseBonus
...
Sub TriggerRight500_Hit()
	If TableTilted=false then
		PlaySound "sensor"
		DOF 113,2
		if TopRolloverLightRight.state=1 then
			SetMotor(3000)
			IncreaseBonus
			If BallsPerGame=3 then
				IncreaseBonus
...
Sub Kicker2Hold_timer()
	...
	If TableTilted=false then
		...
		x=0
		for i = 1 to 10
			if Bonus(i).state=1 then
				x=x+1
			end if
		next
		Kicker2Hold.enabled=false
		Kicker2.TIMERINTERVAL = 150*x
		Kicker2.TIMERENABLED = TRUE
		AddBonusInBall
	else
		Kicker2Hold.enabled=false
...
Sub Spinner1_Spin()
	If TableTilted=false then
		DOF 117,2
		If SpinnerOption=0 then
			AddScore(100)
		else
			AddScore(10)
		end if
		SpinLights(SpinnerCounter).state=0
		SpinnerCounter=SpinnerCounter+1
		If SpinnerCounter>4 then
			IncreaseBonus
			SpinnerCounter=0
		end if
		SpinLights(SpinnerCounter).state=1
...
Sub FlipBumpers()
	if Bumper1Light.state=1 then
		...
		Bumper2Light.state=1
		TopRolloverLightRight.state=1
		If SpecialLitUp=1 then
			LeftOutlaneLight.state=1
			RightOutlaneLight.state=0
		end if
	else
		Bumper2Light.state=0
		TopRolloverLightRight.state=0

		Bumper1Light.state=1
		TopRolloverLightLeft.state=1
		If SpecialLitUp=1 then
			LeftOutlaneLight.state=0
	...
	BumpersOn
	If SpecialLitUp=1 and SpecialEBOption=0 then
		LeftOutlaneLight.state=1
		RightOutlaneLight.state=1
	end if
end sub
...
Sub AddBonus()
	if Bonus(0).state=1 then
		ScoreBonus.interval=400
	else
		ScoreBonus.interval=130
	end if
...
Sub CheckAllDropTargets()
	...
	if TargetsDownCounter=5 then
		SetMotor(5000)
		DropWallResetWait.enabled=true
		if DropTargetsLight.state=1 then
			AddSpecial
		else
			DropTargetsLight.state=1
			SpecialLitUp=1
...
Sub DropWallResetWait_Timer()
	DropWallResetWait.enabled=false
	ResetDropTargets
end sub
...
Sub ResetForNextBall()

	If BallInPlay=3 or BallInPlay=5 then
		Bonus(0).state=1
	else
		Bonus(0).state=0
	end if
	If BallInPlay=1 and DoubleBonusOption=0 then
		Bonus(0).state=1
	end if
	ResetDropTargets
	DropTargetsLight.state=0
	LeftOutlaneLight.state=0
	RightOutlaneLight.state=0
	SpecialLitUp=0
	KickerLight.state=0
...
Sub BumpersOff
	...
	Bumper4Light.visible=false
	Bumper5Light.visible=false
...
Sub BumpersOn
	...
	if Bumper4Light.state=1 then
		Bumper4Light.visible=true
	end if

	if Bumper5Light.state=1 then
		Bumper5Light.visible=true	
	end if
...
sub savehs
	...
	'	scorefile.writeline initials
		ScoreFile.WriteLine BallsPerGame
		ScoreFile.WriteLine SpinnerOption
		ScoreFile.WriteLine DoubleBonusOption
		ScoreFile.WriteLine SpecialEBOption
...
sub loadhs
	...
	Set TextStr=ScoreFile.OpenAsTextStream(1,0)
		...
		SpinnerOption=cdbl(temp4)
		DoubleBonusOption=cdbl(temp5)
		SpecialEBOption=cdbl(temp6)
...
Sub SetupReplayTables
	...
	Replay1Table(3)=66000
	Replay1Table(4)=75000
	Replay1Table(5)=76000
	Replay1Table(6)=78000
	Replay1Table(7)=999000
	Replay1Table(8)=999000
	Replay1Table(9)=999000
	...
	Replay2Table(1)=74000
	Replay2Table(2)=99000
	Replay2Table(3)=78000
	Replay2Table(4)=111000
	Replay2Table(5)=88000
	Replay2Table(6)=87000
	Replay2Table(7)=999000
	Replay2Table(8)=999000
	Replay2Table(9)=999000
	...
	Replay3Table(1)=99000
	Replay3Table(2)=111000
	Replay3Table(3)=111000
	Replay3Table(4)=999000
	Replay3Table(5)=111000
	Replay3Table(6)=131000
...
Sub RefreshReplayCard
	...
	ReplayCard.image = "SC_" + tempst1+tempst2
	Replay1=Replay1Table(ReplayLevel)
	Replay2=Replay2Table(ReplayLevel)
	Replay3=Replay3Table(ReplayLevel)
end sub
...
Sub PlayChime(x)
	if ChimesOn=0 then
		...
		Select Case x
			Case 10
				PlaySound SoundFXDOF("10chime",141,DOFPulse,DOFChimes)
			Case 100
				PlaySound SoundFXDOF("100chime",142,DOFPulse,DOFChimes)	
			Case 1000
				PlaySound SoundFXDOF("1000chime",143,DOFPulse,DOFChimes)
...
Const OptionLine4="Spinner Value"
Const OptionLine5="Double Bonus Balls"
Const OptionLine6="Special/Extra Ball Outlanes"
Const OptionLine7=""
	...
	For linecounter = 1 to MaxOption
		...
		Select Case linecounter
			...
			Case 4:
				...
				If SpinnerOption=0 then
					tempstring = "100 points"
				else 
					tempstring = "10 points"
				end if
			...
			Case 6:
				...
				if SpecialEBOption= 0 then
					tempstring = "Light Both"
				else
					tempstring = "Alternate"
				end if
