' ****************************************************************
'             VP_Cooks " POLTERGEIST" for VISUAL PINBALL X 10.7
'                 Including JP's VPX7 Physics 4
' Table based on the Wrath of the Olympus , BY JP SALAS,
' 
' ****************************************************************
...
Const cGameName = "Poltergeist - VP_Cooks"
...
Sub Table1_Init()
    ...
    End With
    'TV IMAN
        Set TVMagnet = New cvpmMagnet
    With TVMagnet
        .InitMagnet TriggerImanTv, 35
        .GrabCenter = True
        .CreateEvents "TVMagnet"
    End With
...
Sub DisparaBolas
    ...
    Dim dir, speed, ball
    For Each ball In TVMagnet.Balls
        With ball
            dir = (113 * 20.10)/180
            speed = 40 + Rnd * 5
        ...
        End With
    Next
   TVMagnet.MagnetOn = False
End Sub
   ' TRIGGER TV
Sub Trigger006ENCIENDETV_Hit
   FlashForMs FlasherBolatv, 4000, 20, 0
   FlashForMs liTV2, 4000, 20, 0
   Playsound "vo_vocesTV"
   vpmTimer.AddTimer 1000,"Playsound ""vo_hitghost"" '"
   
   DarkNight.visible = 1
   FlasherBolatv.Visible = 1
   FantasmaTimerZ.Enabled = 1 'empieza el movimiento hacia arriba
   Fantasma23TimerZ.Enabled = 1 'empieza el movimiento hacia arriba de los dos fantasmas
   TVMagnet.MagnetOn = True 
    aBallGlowing = True
   vpmTimer.AddTimer 3000, "DisparaBolas: FlasherBolatv.Visible = 0 '"   
   vpmTimer.AddTimer 16000, "aBallGlowing = False: Fantasma2Timer.Enabled = 0: Fantasma3Timer.Enabled = 0: DeactivaFT2: DeactivaFT3: FantasmaTimerZ.Enabled = 1:Fantasma23TimerZ.Enabled = 1: liTV2.State = 0: DarkNight.visible = 0: TVMagnet.MagnetOn = False '"
        ...
        Case 0:
            ...
            Else
                 Select Case Balls
                   ...
                   Case 1: PlaySong "mu_theme1"

                   Case 2: PlaySong "mu_theme2"

                   Case 3: PlaySong "mu_theme3"
...
Sub EndOfBall()
    ...
    If NOT Tilted Then
        ...
        TotalBonus = TotalBonus + AwardPoints
        DMD CL("GHOST COLLECTED " & TreeHits(CurrentPlayer)), CL(FormatScore(AwardPoints)), "", eNone, eBlinkFast, eNone, 750, True, ""
        ...
        TotalBonus = TotalBonus + AwardPoints
        DMD CL("MISSIONS DONE " & TotalMonsters(CurrentPlayer)), CL(FormatScore(AwardPoints)), "", eNone, eBlinkFast, eNone, 750, True, ""

        'Gods defeated x 300,000
        AwardPoints = TotalGods(CurrentPlayer) * 300000
        TotalBonus = TotalBonus + AwardPoints
        DMD CL("BIG MISSIONS DONE " & TotalGods(CurrentPlayer)), CL(FormatScore(AwardPoints)), "", eNone, eBlinkFast, eNone, 750, True, ""
    ...
    If(bGameInPLay = True)AND(Tilted = False)Then
        ...
        If(bBallSaverActive = True)Then
            ...
            If NOT bMultiBallMode Then
                DMD "_", CL("SOUL SAVED"), "_", eNone, eBlinkfast, eNone, 2500, True, "vo_live_again"
...
Sub swPlungerRest_Timer
    IF bOnTheFirstBall Then
        Select Case RndNbr(5)
            ...
            Case 3:DMD CL("I AM CAROL ANNE"), CL("PLAY WITH ME"), "_", eNone, eNone, eNone, 2000, True, "sfx_thunder" &RndNbr(9):FlashEffect 2:ZeusF
...
Sub AwardJackpot()     'only used for the final mode
    DMD CL("JACKPOT"), CL(FormatScore(Jackpot(CurrentPlayer))), "d_border", eNone, eBlinkFast, eNone, 2000, True, "vo_Jackpot" &RndNbr(4)
    DOF 137, DOFPulse
...
Sub ShowTableInfo
    ...
    Else
        ...
        End If
    End If
    DMD "        VP_COOKS", "          AND", "d_jppresents", eNone, eNone, eNone, 2000, False, ""
    DMD "    CAROL ANNE", "  PRESENTS", "d_t800", eNone, eNone, eNone, 2000, False, ""
...
Sub InstantInfo
    ...
    Select Case Mode(CurrentPlayer, 0)
        Case 0 ' no Battle active
        Case 1 ' Minotaur
            DMD CL("CURRENT MODE"), CL("PSYCHOLOGIST"), "", eNone, eNone, eNone, 2000, False, ""
            DMD CL("SHOOT THE LIGHTS"), CL("AND SCOOP TO FINISH"), "", eNone, eNone, eNone, 2000, False, ""
        Case 2 ' Hydra
            DMD CL("CURRENT MODE"), CL("THE FATHER"), "", eNone, eNone, eNone, 2000, False, ""
            DMD CL("SHOOT THE LIGHTS"), CL("AND SCOOP TO FINISH"), "", eNone, eNone, eNone, 2000, False, ""
        Case 3 ' Cerberus
            DMD CL("CURRENT MODE"), CL("THE BROTHER"), "", eNone, eNone, eNone, 2000, False, ""
            DMD CL("SHOOT THE LIGHTS"), CL("AND SCOOP TO FINISH"), "", eNone, eNone, eNone, 2000, False, ""
        Case 4 ' Medusa
            DMD CL("CURRENT MODE"), CL("THE CLOWN"), "", eNone, eNone, eNone, 2000, False, ""
            DMD CL("SHOOT THE LIGHTS"), CL("AND SCOOP TO FINISH"), "", eNone, eNone, eNone, 2000, False, ""
        Case 5 ' Ares
            DMD CL("CURRENT MODE"), CL("REV. KANE"), "", eNone, eNone, eNone, 2000, False, ""
            DMD CL("SHOOT THE SPINNERS"), CL("AND SCOOP TO FINISH"), "", eNone, eNone, eNone, 2000, False, ""
        Case 6 ' Poseidon
            DMD CL("CURRENT MODE"), CL("THE MEDIUM"), "", eNone, eNone, eNone, 2000, False, ""
            DMD CL("SHOOT LIT TARGETS"), CL("AND SCOOP TO FINISH"), "", eNone, eNone, eNone, 2000, False, ""
        Case 7 ' Hades
            DMD CL("CURRENT MODE"), CL("THE INDIAN"), "", eNone, eNone, eNone, 2000, False, ""
            DMD CL("SHOOT THE AFTERLIFE"), "", "", eNone, eNone, eNone, 2000, False, ""
        Case 8 ' Zeus
            DMD CL("CURRENT MODE"), CL("THE MOTHER"), "", eNone, eNone, eNone, 2000, False, ""
            DMD CL("HIT THE DEAD TARGETS"), CL("AND SCOOP TO FINISH"), "", eNone, eNone, eNone, 2000, False, ""
        Case 9 ' God or Demi-God mode
    ...
    DMD CL("MISSIONS DONE"), CL(TotalMonsters(CurrentPlayer)), "", eNone, eNone, eNone, 2000, False, ""
    DMD CL("BIG MISSIONS"), CL(TotalGods(CurrentPlayer)), "", eNone, eNone, eNone, 2000, False, ""
    If Score(1)Then
...
Sub lramp_Hit
    ...
    If bFatesStarted Then 'change the scoring multiplier
        ...
        Select Case RndNbr(3)
            Case 1:FatesMultiplier = 0.5:light006.State = 0
            Case 2:FatesMultiplier = 1:light007.State = 0
            Case 3:FatesMultiplier = 2:light008.State = 0
        End Select
    Else
...
Sub cball_Hit
    ...
    If CBHits(CurrentPlayer)MOD 6 = 0 Then 'open trapdoor for jackpot
        vpmTimer.AddTimer 1500, "PLaySound""vo_trapdoor"":TrapdoorUp '"
    End If
    If CBHits(CurrentPlayer)MOD 15 = 0 Then 'ball breaker
...
Sub CheckPandora
    ...
    If PandorasHits(CurrentPlayer) = PandorasNeeded(CurrentPlayer)Then
        DMD "_", CL("THE GRAVE IS LIT"), "", eNone, eNone, eNone, 1500, True, "vo_mystery"
        li029.State = 1
    End If
    If PandorasHits(CurrentPlayer) >= PandorasNeeded(CurrentPlayer)Then
...
Sub StartPandora
    'do some animation
    DMDFlush
    DMD CL("THE GRAVE"), CL("LIGHT EXTRABALL"), "", eNone, eNone, eNone, 200, False, ""
    ...
    Select case RndNbr(20)
        Case 1, 8 'light extraball
            If Light009.State = 0 Then
                DMD CL("THE GRAVE"), CL("EXTRA BALL IS LIT"), "_", eNone, eBlink, eNone, 2500, True, "vo_extraballislit"
                Light009.State = 2
            Else
                DMD CL("THE GRAVE"), CL(FormatScore(250000)), "_", eNone, eBlink, eNone, 2000, True, ""
                Addscore 250000
            End If
        Case 2, 9 'light special
            If Light010.State = 0 Then
                DMD CL("THE GRAVE"), CL("SPECIAL IS LIT"), "_", eNone, eBlink, eNone, 2500, True, "vo_specialislit"
                Light010.State = 2
            Else
                DMD CL("THE GRAVE"), CL(FormatScore(250000)), "_", eNone, eBlink, eNone, 2000, True, ""
                Addscore2 250000
        ...
        Case 3, 10 'start 3 balls multiball, double playfield scores
            DMD CL("THE GRAVE"), CL("MULTIBALL"), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_multiball"
            DMD "_", CL("PLAYFIELD X 2"), "_", eNone, eBlinkFast, eNone, 2000, True, ""
            AddPlayfieldMultiplier 1
            AddMultiball 2
        Case 4, 11 'award from 250k to 5 million
            tmp = 250000 * RndNbr(20)
            DMD CL("THE GRAVE"), CL(FormatScore(tmp)), "_", eNone, eBlink, eNone, 2000, True, ""
            Addscore2 tmp
        Case 5, 12 'increment bonus multiplier
            AddBonusMultiplier 1
            DMD CL("THE GRAVE"), CL("BONUS X " & BonusMultiplier(CurrentPlayer)), "_", eNone, eBlinkFast, eNone, 2000, True, ""
        Case 6, 13 'add 10 pop bumper values
            BumperAward = BumperAward * 10
            DMD CL("THE GRAVE"), CL("BUMPERS 2X VALUE"), "_", eNone, eNone, eNone, 1000, True, ""
        ...
        Case Else  'hahaha just from 1000 to 25000 points
            tmp = 1000 * RndNbr(25)
            DMD CL("THE GRAVE"), CL(FormatScore(tmp)), "_", eNone, eBlink, eNone, 2000, True, "vo_evilmuah"
            Addscore2 tmp
    End Select
...
Sub StartGodMode
    ...
    DMD CL("GHOST MODE"), CL("FOR " &BallSaverTime& " SECONDS"), "_", eNone, eNone, eNone, 2000, True, "vo_godmode"
    EnableBallSaver BallSaverTime
    PlayThunder
    FlashEffect RndNbr(7)
End Sub
...
Dim Silla1Direccion, Silla1Posicion
...
Sub Silla1Timer_Timer

    Silla1Posicion = Silla1Posicion + Silla1Direccion 

    Silla1.y = Silla1Posicion
    
    If Silla1Posicion <= 1240 Then Silla1Direccion = 2: Silla1Timer.Enabled = 0: vpmtimer.addtimer 3000, "Silla1Timer.Enabled = 1 '"
    If Silla1Posicion >= 1480 Then Silla1Direccion = -2: Silla1Timer.Enabled = 0: vpmtimer.addtimer 3000, "Silla1Timer.Enabled = 1 '"
...
'Posicion inicial, debajo del playfield
FantasmaDireccionZ = 5
FantasmaPosicionZ = -40

Sub FantasmaTimerZ_Timer
    FantasmaTimer.Enabled = 0 'asegúrate que el movimiento está apagado mientras sube y baja
    FantasmaPosicionZ = FantasmaPosicionZ + FantasmaDireccionZ 
    Fantasma.z = FantasmaPosicionZ
    If FantasmaPosicionZ <= -40 Then FantasmaDireccionZ = 5:FantasmaTimerz.Enabled = 0
    If FantasmaPosicionZ >= 260 Then FantasmaDireccionZ = -5:FantasmaTimerz.Enabled = 0:FantasmaTimer.Enabled = 1 'solamente activar el moviemitno horizontal cuando está arriba
End Sub

    '2 FANTASMAS MOVIMIENTOS < >

Dim Fantasma2Direccion, Fantasma2Posicion
Dim Fantasma3Direccion, Fantasma3Posicion

Fantasma2Direccion = 2
Fantasma2Posicion = 400
Fantasma3Direccion = 2
Fantasma3Posicion = 600

Sub Fantasma2Timer_Timer 'mueve el fantasma 2 de izq a dcha. y activa los triggers
    Fantasma2Posicion = Fantasma2Posicion + Fantasma2Direccion 
    Fantasma2.x = Fantasma2Posicion
    If Fantasma2Posicion <= 400 Then Fantasma2Direccion = 2
    If Fantasma2Posicion >= 500 Then Fantasma2Direccion = -2
    ' activa los trigger de abajo del fantasma
    If Fantasma2Posicion >= 395 AND Fantasma2Posicion <=410 Then DeactivaFT2:Ftrig001.IsDropped = 0
    If Fantasma2Posicion >= 420 AND Fantasma2Posicion <=430 Then DeactivaFT2:Ftrig002.IsDropped = 0
    If Fantasma2Posicion >= 445 AND Fantasma2Posicion <=455 Then DeactivaFT2:Ftrig003.IsDropped = 0
    If Fantasma2Posicion >= 470 AND Fantasma2Posicion <=480 Then DeactivaFT2:Ftrig004.IsDropped = 0
    If Fantasma2Posicion >= 490 AND Fantasma2Posicion <=505 Then DeactivaFT2:Ftrig005.IsDropped = 0
End Sub

Sub Ftrig2_Hit(idx): PlaySoundAt"sfx_hitghost", fantasma2: AwardFantasmaJackpot:Fantasma2.Color =255: vpmtimer.AddTimer 1000,"fantasma2.color = 16776960 '": End Sub
Sub Ftrig3_Hit(idx): PlaySoundAt"sfx_hitghost", fantasma3: AwardFantasmaJackpot:Fantasma3.Color =255: vpmtimer.AddTimer 1000,"fantasma3.color = 16776960 '": End Sub

Sub AwardFantasmaJackpot()
    DMD CL("GHOST JACKPOT"), CL(FormatScore(Jackpot(CurrentPlayer))), "d_border", eNone, eBlinkFast, eNone, 2000, True, "vo_Jackpot" &RndNbr(4)
    DOF 137, DOFPulse
    AddScore2 Jackpot(CurrentPlayer)
    Jackpot(CurrentPlayer) = Jackpot(CurrentPlayer) + 100000
    LightEffect 2
...
Sub DeactivaFT2
    Ftrig001.IsDropped = 1
    Ftrig002.IsDropped = 1
    Ftrig003.IsDropped = 1
    Ftrig004.IsDropped = 1
    Ftrig005.IsDropped = 1
End Sub

Sub Fantasma3Timer_Timer 'mueve el fantasma 3 de izq a dcha. y activa los triggers
    Fantasma3Posicion = Fantasma3Posicion + Fantasma3Direccion 
    Fantasma3.x = Fantasma3Posicion
    If Fantasma3Posicion <= 600 Then Fantasma3Direccion = 2
    If Fantasma3Posicion >= 700 Then Fantasma3Direccion = -2
    ' activa los trigger de abajo del fantasma
    If Fantasma3Posicion >= 595 AND Fantasma3Posicion <=610 Then DeactivaFT3:Ftrig006.IsDropped = 0
    If Fantasma3Posicion >= 620 AND Fantasma3Posicion <=630 Then DeactivaFT3:Ftrig007.IsDropped = 0
    If Fantasma3Posicion >= 645 AND Fantasma3Posicion <=655 Then DeactivaFT3:Ftrig008.IsDropped = 0
    If Fantasma3Posicion >= 670 AND Fantasma3Posicion <=680 Then DeactivaFT3:Ftrig009.IsDropped = 0
    If Fantasma3Posicion >= 690 AND Fantasma3Posicion <=705 Then DeactivaFT3:Ftrig010.IsDropped = 0
End Sub

Sub DeactivaFT3
    Ftrig006.IsDropped = 1
    Ftrig007.IsDropped = 1
    Ftrig008.IsDropped = 1
    Ftrig009.IsDropped = 1
    Ftrig010.IsDropped = 1
End Sub

    '2 FANTASMAS MOVIMIENTO eje z
Dim Fantasma2DireccionZ, Fantasma2PosicionZ
 
'Posicion inicial, debajo del playfield
Fantasma2DireccionZ = 2
Fantasma2PosicionZ = -40

Sub Fantasma23TimerZ_Timer
    Fantasma2Timer.Enabled = 0 'asegúrate que el movimiento está apagado mientras sube y baja
    Fantasma3Timer.Enabled = 0 'asegúrate que el movimiento está apagado mientras sube y baja
    Fantasma2PosicionZ = Fantasma2PosicionZ + Fantasma2DireccionZ 
    Fantasma2.z = Fantasma2PosicionZ
    Fantasma3.z = Fantasma2PosicionZ
    If Fantasma2PosicionZ <= -40 Then Fantasma2DireccionZ = 2:Fantasma23Timerz.Enabled = 0
    If Fantasma2PosicionZ >= 60 Then Fantasma2DireccionZ = -2:Fantasma23Timerz.Enabled = 0:Fantasma2Timer.Enabled = 1:Fantasma3Timer.Enabled = 1 'solamente activar el moviemitno horizontal cuando está arriba
End Sub




'Flashers manos indio x DERECHA

Dim FlasherManos1Direccion, FlasherManos1Posicion

 

FlasherManos1Direccion = -2

FlasherManos1Posicion = 480
 

Sub FlasherManos1Timer_Timer

    FlasherManos1Posicion = FlasherManos1Posicion + FlasherManos1Direccion 

    FlasherManos1.x = FlasherManos1Posicion

 

    If FlasherManos1Posicion <= 478 Then FlasherManos1Direccion = 2

    If FlasherManos1Posicion >= 480 Then FlasherManos1Direccion = -2

End Sub

'Flashers manos indio x IZQUIERDA

Dim FlasherManos2Direccion, FlasherManos2Posicion

 

FlasherManos2Direccion = -1

FlasherManos2Posicion = 350

 

Sub FlasherManos2Timer_Timer

    FlasherManos2Posicion = FlasherManos2Posicion + FlasherManos2Direccion 

    FlasherManos2.x = FlasherManos2Posicion

 

    If FlasherManos2Posicion <= 348 Then FlasherManos2Direccion = 2

    If FlasherManos2Posicion >= 350 Then FlasherManos2Direccion = -2

End Sub



'Flashers manos indio EJE Y DERECHA

Dim FlasherManos1DireccionY, FlasherManos1PosicionY

 

FlasherManos1DireccionY = -2

FlasherManos1PosicionY = 533

 

Sub FlasherManos1TimerY_Timer

    FlasherManos1PosicionY = FlasherManos1PosicionY + FlasherManos1DireccionY 

    FlasherManos1.Y = FlasherManos1PosicionY

 

    If FlasherManos1PosicionY <= 532 Then FlasherManos1DireccionY = 2

    If FlasherManos1PosicionY >= 536 Then FlasherManos1DireccionY = -2

End Sub

'Flashers manos indio EJE Y IZQUIERDA

Dim FlasherManos2DireccionY, FlasherManos2PosicionY

 

FlasherManos2DireccionY = -1

FlasherManos2PosicionY = 641

 

Sub FlasherManos2TimerY_Timer

    FlasherManos2PosicionY = FlasherManos2PosicionY + FlasherManos2DireccionY 

    FlasherManos2.Y = FlasherManos2PosicionY

 

    If FlasherManos2PosicionY <= 637 Then FlasherManos2DireccionY = 2

    If FlasherManos2PosicionY >= 641 Then FlasherManos2DireccionY = -2
    ...
    Select Case Mode(CurrentPlayer, 0)
        Case 0 'no battle is active
            ...
            If tmp = 4 OR(OrbitHits + RampHits = 4)Then
                DMD "_", CL("START MISSION IS LIT"), "_", eNone, eBlink, eNone, 2500, True, ""
                bModeReady = True
...
Sub StartNextMode 'starts the mode in the Mode(CurrentPlayer, 0) variable
    ...
    Select Case Mode(CurrentPlayer, 0)
        Case 1 ' Minotaur
            ...
            DMD CL("PSYCHOLOGIST STARTED"), CL("HIT LIT SHOTS"), "d_minotaur", eNone, eBlink, eNone, 2500, True, "vo_Minotaur-mode"
        ...
        Case 2 ' Hydra
            ...
            DMD CL("THE FATHER STARTED"), CL("HIT LIT SHOTS"), "d_hydra", eNone, eBlink, eNone, 2500, True, "vo_Hydra-mode"
...
'Must hit following shots:  left ramp, right ramp, and upper ramp, (any order) final shot being the center scoop.
            DMD CL("THE BROTHER STARTED"), CL("HIT LIT SHOTS"), "d_cerberus", eNone, eBlink, eNone, 2500, True, "vo_Cerberus-mode"
...
'Completing the mode awards a five ball Medusa Multi-ball. 15 seconds ball saver.
            DMD CL("THE CLOWN STARTED"), CL("HIT LIT SHOTS"), "d_medusa", eNone, eBlink, eNone, 2500, True, "vo_Medusa-mode"
        ...
        Case 5 ' Ares
            ...
            End If
            DMD CL("THE MEDIUM STARTED"), CL("HIT THE SPINNERS"), "d_ares", eNone, eBlink, eNone, 2500, True, "vo_Ares-mode"
        ...
        Case 6 ' Poseidon
            ...
            If bJackpotsEnabled Then
                ...
                li040.State = 0
            End If
            DMD CL("THE MEDIUM STARTED"), CL("HIT THE LIT TARGETS"), "d_poseidon", eNone, eBlink, eNone, 2500, True, "vo_poseidon_mode"
        ...
        Case 7 ' Hades
            ...
            If li012.State = 1 AND li011.State = 1 Then
                AwardExtraBall
            End If
            DMD CL("THE INDIAN STARTED"), CL("HIT THE TRAPDOOR"), "d_hades", eNone, eBlink, eNone, 2500, True, "vo_hades_mode"
        ...
        Case 8 ' Zeus
            ...
            If li012.State = 1 AND li014.State = 1 Then
                ...
                Light010.State = 2
            End If
            DMD CL("THE MOTHER STARTED"), CL("SPELL ZEUS 3 TIMES"), "d_zeus", eNone, eBlink, eNone, 2500, True, "vo_specialislit"
        ...
        Case 9                       ' Wizards modes: God or Demi-God mode
            ...
            If tmp = 8 Then 'all lights are lit and the battles are all completed
                DMD CL("GHOST MODE STARTED"), CL("HIT THE JACKPOTS"), "d_zeus", eNone, eBlink, eNone, 2500, True, ""
                EndModeCountdown = 120
            Else
                DMD CL("PRE WIZRD MODE STARTED"), CL("HIT THE JACKPOTS"), "d_zeus", eNone, eBlink, eNone, 2500, True, ""
                EndModeCountdown = 60
            End If
...
Sub WinMode 'when you complete all the tasks
    ...
    Select Case Mode(CurrentPlayer, 0)
        Case 1                         ' Minotaur
            ...
            DMDFlush
            DMD "_", CL("PSYCHOLOGYST DONE"), "_", eNone, eScrollLeft, eNone, 20, True, "sfx_thunder" & RndNbr(9)
            DMD "_", CL("PSYCHOLOGYST DONE"), "_", eNone, eBlinkFast, eNone, 1500, True, ""
        ...
        Case 2 ' Hydra
            ...
            TotalMonsters(CurrentPlayer) = TotalMonsters(CurrentPlayer) + 1
            DMDFlush
            DMD "_", CL("THE FATHER DONE"), "_", eNone, eScrollLeft, eNone, 20, True, "sfx_thunder" & RndNbr(9)
            DMD "_", CL("THE FATHER DONE"), "_", eNone, eBlinkFast, eNone, 1500, True, ""
            DMD "_", SPACE(20), "_", eNone, eScrollLeft, eNone, 20, True, ""
            ModeStep = 0
            AddScore2 1000000
            TurnOffHelmet
        ...
        Case 3 ' Cerberus
            ...
            TotalMonsters(CurrentPlayer) = TotalMonsters(CurrentPlayer) + 1
            DMDFlush
            DMD "_", CL("THE BROTHER DONE"), "_", eNone, eScrollLeft, eNone, 20, True, "sfx_thunder" & RndNbr(9)
            DMD "_", CL("THE BROTHER DONE"), "_", eNone, eBlinkFast, eNone, 1500, True, ""
            DMD "_", SPACE(20), "_", eNone, eScrollLeft, eNone, 20, True, ""
            ModeStep = 0
            AddScore2 1000000
            TurnOffHelmet
        ...
        Case 4 ' Medusa
            ...
            TotalMonsters(CurrentPlayer) = TotalMonsters(CurrentPlayer) + 1
            DMDFlush
            DMD "_", CL("THE CLOWN DONE"), "_", eNone, eScrollLeft, eNone, 20, True, "sfx_thunder" & RndNbr(9)
            DMD "_", CL("THE CLOWN DONE"), "_", eNone, eBlinkFast, eNone, 1500, True, ""
            DMD "_", SPACE(20), "_", eNone, eScrollLeft, eNone, 20, True, ""
            ModeStep = 0
            AddScore2 1000000
            TurnOffHelmet
            ...
            If TotalMonsters(CurrentPlayer) = 4 Then 'all monsters have been defeated, turn on the beast master light
                DMD "YOU ARE THE", CL("GHOST MASTER"), "_", eNone, eNone, eNone, 1500, True, "vo_beast_master"
                li047.State = 1
                AddScore2 1000000
            End If
        Case 5 ' Ares
            ...
            DMDFlush
            DMD "_", CL("REV. KANE DONE"), "_", eNone, eScrollLeft, eNone, 20, True, "sfx_thunder" & RndNbr(9)
            DMD "_", CL("REV. KANE DONE"), "_", eNone, eBlinkFast, eNone, 2000, True, ""
            DMD "_", SPACE(20), "_", eNone, eScrollLeft, eNone, 20, True, ""
            ModeStep = 0
            AddScore2 1000000
            TurnOffHelmet
        ...
        Case 6 ' Poseidon
            ...
            TotalGods(CurrentPlayer) = TotalGods(CurrentPlayer) + 1
            DMDFlush
            DMD "_", CL("THE MEDIUM DONE"), "_", eNone, eScrollLeft, eNone, 20, True, "sfx_thunder" & RndNbr(9)
            DMD "_", CL("THE MEDIUM DONE"), "_", eNone, eBlinkFast, eNone, 2000, True, ""
            DMD "_", SPACE(20), "_", eNone, eScrollLeft, eNone, 20, True, ""
            ModeStep = 0
            AddScore2 1000000
            TurnOffHelmet
        ...
        Case 7 ' Hades
            ...
            TotalGods(CurrentPlayer) = TotalGods(CurrentPlayer) + 1
            DMDFlush
            DMD "_", CL("THE INDIAN DONE"), "_", eNone, eScrollLeft, eNone, 20, True, "sfx_thunder" & RndNbr(9)
            DMD "_", CL("THE INDIAN DONE"), "_", eNone, eBlinkFast, eNone, 2000, True, ""
            DMD "_", SPACE(20), "_", eNone, eScrollLeft, eNone, 20, True, ""
            ModeStep = 0
            AddScore2 5000000
            TurnOffHelmet
        ...
        Case 8 ' Zeus
            ...
            TotalGods(CurrentPlayer) = TotalGods(CurrentPlayer) + 1
            DMDFlush
            DMD "_", CL("THE MOTHER"), "_", eNone, eScrollLeft, eNone, 20, True, "sfx_thunder" & RndNbr(9)
            DMD "_", CL("THE MOTHER"), "_", eNone, eBlinkFast, eNone, 2000, True, ""
            DMD "_", SPACE(20), "_", eNone, eScrollLeft, eNone, 20, True, ""
            ModeStep = 0
            AddScore2 5000000
            TurnOffHelmet
            ...
            If TotalGods(CurrentPlayer) = 4 Then 'all gods has been defeated
                DMD "YOU ARE THE", CL("GHOST KING"), "_", eNone, eNone, eNone, 1500, True, "vo_god_of_gods"
                AddScore2 2000000
            End If
...
Sub EndModeTimer_Timer '1 second timer to count down to end the timed modes
    ...
    Select Case EndModeCountdown
        ...
        Case 0
            DMD CL("TIME IS UP"), CL("MISSION TERMINATED"), "_", eNone, eBlinkFast, eNone, 1500, True, ""
            If Mode(CurrentPlayer, 0) = 9 Then
                DMD CL("GET READY"), CL("THE GHOSTS ARE BACK"), "_", eNone, eBlinkFast, eNone, 2000, True, ""
            Else
...
Sub StartMedussaMB
    DMD CL("THE FATHER MULTIBALL"), CL("SHOOT THE LOOPS"), "_", eNone, eNone, eNone, 1500, True, "vo_medusa_multiball"
...
Sub CheckMinotaurMB
    ...
    If BallsInLock(CurrentPlayer) = 3 Then
        DMD CL("THE BROTHER MULTIBALL"), CL("SHOOT THE HOLES"), "_", eNone, eNone, eNone, 1500, True, "vo_minotaur_multiball"
...
Sub CheckMinotaurMBHits
    If Minotaur1 + Minotaur2 + Minotaur3 = 3 Then 'all 3 holes has been hit so double the jackpot
        ...
        DMD CL("BROTHER JACKPOT IS"), CL(FormatScore(MinotaurJackpot(CurrentPlayer))), "_", eNone, eNone, eNone, 1500, True, ""
        Minotaur1 = 0
        Minotaur2 = 0
        Minotaur3 = 0
    End If
...
Sub AwardMinotaurJackpot()
    DOF 130, DOFPulse
    DMD CL("BROTHER JACKPOT"), CL(FormatScore(MinotaurJackpot(CurrentPlayer))), "_", eNone, eBlinkFast, eNone, 2000, True, "vo_Jackpot" &RndNbr(4)
    DOF 126, DOFPulse
    AddScore2 Jackpot(CurrentPlayer)
...
Sub StartZeusMB
    DMD CL("THE MOTHER MULTIBALL"), "SHOOT THE UPPER RAMP", "_", eNone, eNone, eNone, 1500, True, "vo_zeus_multiball"
    bZeusMBStarted = True
    AddMultiball 3
    EnableBallSaver 15
...
Sub CheckFates 'checks for hits and start the Fates mode
    If FatesHits(CurrentPlayer)MOD 10 = 0 Then
        DMD CL("CEMETARY"), "STARTED", "_", eNone, eNone, eNone, 1500, True, "vo_The_fates"
        bFatesStarted = True
        li035.State = 2
    End If
End Sub
