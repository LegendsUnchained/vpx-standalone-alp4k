...
Const BallMass = 0.9    ' 1 is the normal mass
Const BallMass1 = 0.5    ' 1 is the normal mass
...
'2 Ball Superman
    ...
    T2ballsuper1.Enabled = 1
    T2ballsuper2.Enabled = 0
    T2ballsuper3.Enabled = 0
    T2ballsuper4.Enabled = 0

Sub T2ballsuper1_hit()
    T2ballsuper1.Enabled = 0
    T2ballsuper2.Enabled = 1
    light12a.state = 1
    Addscore 10000
    Check2ball
End Sub

Sub T2ballsuper2_hit()
    T2ballsuper2.Enabled = 0
    T2ballsuper3.Enabled = 1
    light13a.state = 1
    Addscore 10000
    Check2ball
End Sub

Sub T2ballsuper3_hit()
    T2ballsuper3.Enabled = 0
    T2ballsuper4.Enabled = 1
    light14a.state = 1
    Addscore 10000
    Check2ball
End Sub

Sub T2ballsuper4_hit()
    T2ballsuper4.Enabled = 0
    light15a.state = 1
    Addscore 10000
    Check2ball
...
Sub Check2ball()
    If light12a.state + light13a.state + light14a.state + light15a.state = 4 Then
        light12a.state = 0
        light13a.state = 0
        light14a.state = 0
        light15a.state = 0
        Gate2.Open = 0
        supermanball.visible = 1
        Ramp2ball.collidable = 0
        Targetcabine.isdropped = 1
        Lightphone.State = 2
        PlaySound "Fxgomultiball"
        Addscore 30000
        End If
End Sub

sub Kicker002_hit()
  Kicker002.destroyball : Playsound "Fxmultiball"
  T2ballsuper1.Enabled = 1
  Lightphone.state = 0
  Ramp2ball.collidable = 1
  supermanball.visible = 0
  Gate2.Open = 1
  Targetcabine.isdropped = 0
  kicker001.createball
  kicker001.kick 90,22,80 : Playsound "power-up"
  AddMultiball 1
  'ChangeBall(3)
end Sub
...
Set mMagnaSave3 = New cvpmMagnet : With mMagnaSave3
	.InitMagnet magna3, 1
End With
...
Sub recordspin_timer
	'recordplate.RotZ = recordplate.RotZ + 0.3
    roue.RotZ = roue.RotZ + 0.3
    planet.RotY = planet.RotY + 0.2
    Mysteryman.RotY = Mysteryman.RotY + 0.1
End Sub
...
Sub Checksuper()
  If LletterS.state + LletterU.state + LletterP.state + LletterE.state + LletterR.state + LletterM.state + LletterA.state + LletterN.state  = 8 Then
     Playsound "soundfx27"
     LletterS.state = 0
     LletterU.state = 0
     LletterP.state = 0
...
sub Timersaveball_timer
	...
	KsaveR.kick 0, 90 : PlaySound "elevatorclose"
    PlaySound "issupermanreturns"
end sub


sub jackpotcroco_hit() : DMD CL("KENT"), CL("SUPER JACKPOT"), "", eBlink, eBlink, eNone, 3000, True, ""
...
Sub CheckJL()
    If lightJL1.State + lightJL2.State + lightJL3.State + lightJL4.State + lightJL5.State + lightJL6.State + lightJL7.State + lightJL8.State + lightJL9.State = 9 Then
       Kcroco.enabled = 1
       lightJL.State = 2
       AddScore 20000
    End IF
...
sub Timercroco_timer
    ...
    PlaySound "issuperman"
	Kcroco.kick 170 , 10 : DMD CL("SUPERMAN"), CL("MULTIBALL"), "", eBlink, eBlink, eNone, 3000, True, ""
    TargetJL1.IsDropped = 0
    TargetJL2.IsDropped = 0
    TargetJL3.IsDropped = 0
    TargetJL4.IsDropped = 0
    TargetJL5.IsDropped = 0
    TargetJL6.IsDropped = 0
    TargetJL7.IsDropped = 0
    TargetJL8.IsDropped = 0
    TargetJL9.IsDropped = 0
    lightJL1.State = 0
    lightJL2.State = 0
    lightJL3.State = 0
    lightJL4.State = 0
    ...
    FlashForMs fcroco2, 1900, 50, 0
    FlashForMs fcroco3, 2100, 50, 0
    FlashForMs fcroco4, 2300, 50, 0
    AddMultiball 2
    LightJL.State = 0
    AddScore 20000
    ChangeBall(3)
...
Sub Table1_KeyUp(ByVal keycode)
    ...
    if keycode=21 then roue.rotz=20 
    if keycode=23 then roue.transz=100

    If keycode = PlungerKey Then
        Plunger.Fire
        PlaySoundAt "fx_plunger", plunger
        If bBallInPlungerLane Then PlaySoundAt "SE260", plunger
    End If
...
Sub InstantInfo
    ...
    DMD CL("LOCKED BALLS"), CL(BallsInLock1(CurrentPlayer)), "", eNone, eNone, eNone, 800, False, ""
...
sub Targetroue_hit()
	...
	kicker015.enabled=1
	roueTimer.enabled=1
    Wallroue.collidable = 0
end sub

Sub roueTimer_Timer
   roue.rotz = roue.rotz + 1
   roue.transz = 100
   Lightopenroue.state = 2
   Lightopenroue1.state = 2
   if roue.rotz > 10000 then
       roue.rotz = 1
       roue.transz = 0
		rouetimer.enabled=0
		kicker015.enabled=0
   Lightopenroue.state = 0
   Lightopenroue1.state = 0
   end if
end sub

sub Kicker015_hit()
    kicker015.enabled=0
    'Wallroue.collidable = 1
	Kicker015.destroyball: Wallroue.collidable = 1 : Playsound "fx_balldrop" : FlashForMs Fback, 1500, 50, 0
    Kicker003.createball : Playsound "superman-dc"
	Timer003.enabled=1
end sub
...
sub timer003_timer
	Timer003.enabled=0
	Kicker003.kick 45, 8 : Playsound "superman-dc"
    Lightroue.state = 2
end sub
...
Sub swPlungerRest_Hit()
    ...
    If bAutoPlunger Then
        ...
        PlungerIM.AutoFire
        DOF 117, DOFPulse
        PlaySoundAt "SE260", swPlungerRest
        bAutoPlunger = False
    End If
...
Sub DMDScore()
    ...
    if(dqHead = dqTail)Then
        ...
        Select Case Battle(CurrentPlayer, 0)
            ...
            Case 2:tmp1 = CL("HIT THE BUMPERS " & 20-SuperBumperHIts)
            Case 3:tmp1 = CL("HIT THE RAMPS " & 6-ramphits3)
            Case 4:tmp1 = CL("HIT LANE " & 6-orbithits)
...
Sub ShowTableInfo
    ...
    DMD "", CL("TRUTH AND JUSTICE" &myversion), "", eNone, eNone, eNone, 2000, False, ""
...
Sub UpdateSkillShot() 'Setup and updates the skillshot lights
    ...
    Light21.State = 2
    Light29.State = 2
    Rampstart.collidable = 0
    wallopen.collidable = 0
    Gate2.Open = 1
    'Gate3.Open = 1
...
Sub ResetSkillShotTimer_Timer 'timer to reset the skillshot lights & variables
    ...
    Light29.State = 0
    Rampstart.collidable = 1
    wallopen.collidable = 1
    'Gate2.Open = 0
    'Gate3.Open = 0
    DMDScoreNow
End Sub
...
Sub Trees_Timer()
    ...
    'Tree15.RotZ = - TreeDir *4
     supermanball.RotY = - TreeDir *15
     'couteauD.RotZ = - TreeDir *15
End Sub

Sub BoatTimer_Timer()
    BoatDir = SIN(BoatStep * MyPi)
    BoatStep = (BoatStep + 1)MOD 360
    supermanball.Z = supermanball.Z + BoatDir * 3
    'supermanball.Y = supermanball.Y + BoatDir * 5
End Sub
...
Sub Target10_Hit
    PlaySoundAt SoundFXDOF("SE109", 116, DOFPulse, DOFTargets), Target10
    If Tilted Then Exit Sub
    AddScore 5000
    TargetBonus = TargetBonus + 1
    ' Do some sound or light effect
...
Sub Target11_Hit
    PlaySoundAt SoundFXDOF("SE109", 116, DOFPulse, DOFTargets), Target11
    If Tilted Then Exit Sub
    AddScore 5000
    TargetBonus = TargetBonus + 1
    ' Do some sound or light effect
...
Sub lock_Hit
    ...
    If light27.State = 1 Then 'lock the ball
        ...
        Select Case BallsInLock(CurrentPlayer)
            Case 1:DMD "_", CL("BALL 1 LOCKED"), "_", eNone, eBlinkFast, eNone, 1500, True, "vo_oneballlock" : Lock1.State = 1
            Case 2:DMD "_", CL("BALL 2 LOCKED"), "_", eNone, eBlinkFast, eNone, 1500, True, "vo_twoballlock" : Lock2.State = 1
            Case 3:DMD "_", CL("BALL 3 LOCKED"), "_", eNone, eBlinkFast, eNone, 1500, True, "vo_threeballlock" : Lock3.State = 1
        End Select
        light27.State = 0
...
Sub ReleaseLockedBall 'release locked ball
    ...
    DOF 121, DOFPulse
    lock.kick 180, 10 : PlaySoundAt SoundFXDOF("fx_solenoid", 142, DOFPulse, DOFContactors), lock
...
Sub Check5BankTargets
    ...
    If light15.state + light16.state + light17.state + light18.state + light19.state = 5 Then
        ...
        DMD "YOU HAVE FOUND", "THE OBJECTS", "monster_" &tmp, eNone, eNone, eNone, 2500, True, "splendid1"
        LightEffect 1
        FlashEffect 1
...
Sub Target6_Hit
    PlaySoundAtBall SoundFXDOF("fxredtarget", 113, DOFPulse, DOFTargets)
    If Tilted Then Exit Sub
    If bSkillShotReady Then
        Awardskillshot
        Exit Sub
...
Sub GiveRandomAward() 'from the Pyramid
    ...
    DMD CL("SUPERMAN AWARD"), "", "", eNone, eNone, eNone, 50, False, "fx_spinner"
    ...
    Select Case tmp
        ...
        Case 19, 20, 21 'Complete Battle
            If Battle(CurrentPlayer, 0) > 0 AND Battle(CurrentPlayer, 0) < 13 Then
                DMD CL("MISSION"), CL("COMPLETED"), "", eBlink, eBlink, eNone, 1500, True, "fx_fanfare1"
                WinBattle
            Else
                DMD CL("BIG POINTS"), CL("100000"), "", eBlink, eBlink, eNone, 1500, True, "fx_fanfare1"
                AddScore 100000
...
Sub RightRampDone_Hit
    ...
    If light31.State = 2 Then
        DMD CL("SUPER HEROS"), CL(jackpot(CurrentPlayer)), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_Jackpot"
        AddScore Jackpot(CurrentPlayer)
        LightEffect 2
        FlashEffect 2
    Else
...
' This table has 12 main battles, 3 wizard battles, and a final battle
...
Sub SelectBattle 'select a new random battle if none is active
    ...
    If Battle(CurrentPlayer, 0) = 0 Then
        ...
        Select Case BattlesWon(CurrentPlayer)
            ...
            Case 14:NewBattle = 15:Battle(CurrentPlayer, NewBattle) = 2:UpdateBattleLights:StartBattle '12 battles + 3 wizard modes = start final battle
            Case Else
...
Sub UpdateBattleLights
    ...
    Light41a.State = Battle(CurrentPlayer, 12)
...
Sub checkchildren()
If Light38b.State  + Light7b.State + Light64a.State + Light41a.State + Light35b.State = 5 Then
   AddScore 500000
   AddMultiball 5
End If
End Sub
...
Sub StartBattle
    ...
    Select Case NewBattle
        Case 1 'Super Spinners
            DMD CL("SUPER GIRL"), CL("SHOOT THE SPINNERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light45a.State = 2
            Light70.State = 2
            SpinCount = 0
        Case 2 'Super Pop Bumpers
            DMD CL("LOIS LANE"), CL("SHOOT BUMPER"), "", eNone, eNone, eNone, 1500, True, ""
            Light22.State = 2
            LightSeqBumpers.Play SeqRandom, 10, , 1000
            SuperBumperHits = 0
        Case 3 'Ramps
            DMD CL("CYBORG"), CL("SHOOT THE RAMPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits3 = 0
        Case 4 'Batman = Orbits
            DMD CL("BATMAN"), CL("SHOOT THE ORBITS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            OrbitHits = 0
            StartSpots
        Case 5 'Shoot the lights 2
            DMD CL("FLASH"), CL("SHOOT THE LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light45.State = 2
            Light46.State = 2
            Light47.State = 2
            ...
            Light49.State = 2
            Light50.State = 2
        Case 6 'Shoot the lights 1
            DMD CL("AQUAMAN"), CL("SHOOT THE LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
        Case 7 'Target Frenzy
            DMD CL("WONDERWOMAN"), CL("ALL TARGETS-RAMPS"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqAllTargets.Play SeqRandom, 10, , 1000
            TargetHits7 = 0
        Case 8 'Left & Right Targets
            DMD CL("LUTHOR"), CL("TARGETS AND RAMPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light50.State = 2
            TargetHits8 = 0
        Case 9 'Target JL
            DMD CL("LASER EYES"), CL("SHOOT THE BALL"), "", eNone, eNone, eNone, 1500, True, ""
            Light29.State = 2
            CaptiveBallHits = 0
        Case 10 'Ramps and Orbits
            'uses the ramphits10 to count the hits
            DMD CL("GREEN LANTERN"), CL("SHOOT RAMP AND LANE"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits10 = 0
        Case 11 'Blue Targets
            DMD CL("GENERAL ZOD"), CL("SHOOT BLUE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqBlueTargets.Play SeqRandom, 10, , 1000
            TargetHits11 = 0
        Case 12 'Super Loops
            DMD CL("KRIPTONITE"), CL("SHOOT THE LOOPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            'Gate2.Open = 1
            'Gate3.Open = 1
            kripto.visible = 0 'Modified so ALP4k won't crash
            Rampstart.collidable = 0
            wallopen.collidable = 0
            loopCount = 0
        Case 13 'Follow the Lights 1
            DMD CL("METALLO"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            FollowTheLights.Enabled = 1
            AddMultiball 2
            StartJackpots
            ChangeGi green
        Case 14 'Follow the Lights 2
            DMD CL("JUSTICE LEAGUE"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            FollowTheLights.Enabled = 1
            AddMultiball 3
            StartJackpots
            ChangeGi purple
        Case 15 'the final battle
            DMD CL("SUPERMAN"), CL("SHOOT THE JACKPOTS"), "", eNone, eNone, eNone, 1500, True, ""
            AddMultiball 4
            StartJackpots
            ChangeGi yellow
    End Select
...
Sub CheckWinBattle
    ...
    Select Case NewBattle
        ...
        Case 2
            If SuperBumperHits = 20 Then WinBattle:End if
        Case 3
        ...
        Case 12
            If loopCount = 6 Then WinBattle:End if
    End Select
End Sub
...
Sub WinBattle
    ...
    LightEffect 2
    GiEffect 2
    DMD "", CL("MISSION COMPLETED"), "_", eNone, eBlinkFast, eNone, 1000, True, "winbattle"
    DOF 139, DOFPulse
    tmp = INT(RND * 4)
    Select Case tmp
...
Sub StopBattle2
    ...
    Select Case NewBattle
        ...
        Case 12:Rampstart.collidable = 1 : wallopen.collidable = 1 : kripto.visible = 0
