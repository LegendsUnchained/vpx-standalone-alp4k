...
'                 Halloween Script JPSalas & marty
'                          Version 4.0
' ****************************************************************
...
Sub bluetimer_timer
    Fblue.imageA = "bluelight" &i1
    'Fblue.imageA = "torch" &i2
    'Fblue.imageA = "torch" &i4
...
mikeatimer.enabled = 1
mikebtimer.enabled = 1
mikectimer.enabled = 1
mikeclosetimer.enabled = 1

sub mikemove_hit()
	mikeatimer.enabled=1
    PlaySound "rirehorror"
end sub

sub mikeatimer_timer
	mikeatimer.enabled=0
    mikea.RotY = mikea.RotY + 180
end sub
...
sub mikebtimer_timer
	mikebtimer.enabled=0
    mikeb.RotY = mikeb.RotY + 180
end sub
...
sub mikectimer_timer
	mikectimer.enabled=0
    mikec.RotY = mikec.RotY + 180
end sub
...
sub mikeclosetimer_timer
	mikeclosetimer.enabled=0
    mikea.RotY = mikea.RotY - 180
    mikeb.RotY = mikeb.RotY - 180	
    mikec.RotY = mikec.RotY - 180
end sub
...
sub K1D_hit()
	...
	Kicker004.kick 170,10
end sub

sub K1G_hit()
	K1G.destroyball
    PlaySound "crie_femme" : pupevent 832
	Kicker003.createball
	Kicker003.kick 180,10
end sub

sub K2G_hit()
	K2G.destroyball
    PlaySound "crie_femme1" : pupevent 833
	Kicker003.createball
	Kicker003.kick 180,10
end sub
...
sub K3G_hit()
	K3G.destroyball
    PlaySound "crie_femme2" : pupevent 833
	Kicker003.createball
	Kicker003.kick 180,10
end sub

sub pontup_hit()
pont.RotZ = pont.RotZ +30
end sub
...
Const HolostepMax = 100
'Dim Holostep1:Holostep1 = 1
Dim Holostep2:Holostep2 = 1

Sub HologramAnim2_Timer()
    HologramAnim2.interval = 100
    'sang.visible = 1



    if Holostep2 < 10 then 
        imagename = "sang" & "00" & Holostep2
    elseif Holostep2 < 100 then
        imagename = "sang" & "0" & Holostep2 
    else 
        imagename = "sang" & Holostep2 
    end if

    sang.imageA = imagename
    Holostep2 = Holostep2 + 1 

    If Holostep2 > HolostepMax Then
        Holostep2 = 1
    End If
...
Sub Table1_KeyDown(ByVal Keycode)
    ...
    If Keycode = AddCreditKey Then
        ...
        If(Tilted = False)Then
            DMDFlush
            DMD "_", CL("CREDITS: " & Credits), "", eNone, eNone, eNone, 500, True, "fx_coin" : pupevent 100
...
Sub CreateNewBall()
    ...
    BallRelease.CreateSizedBallWithMass BallSize / 2, BallMass : pupevent 101
...
Sub EndOfBall()
    Dim AwardPoints, TotalBonus, ii : pupevent 817
    AwardPoints = 0
    TotalBonus = 0
...
Sub EndOfBall2()
    ...
    If(ExtraBallsAwards(CurrentPlayer) <> 0)Then
        ...
        DMD CL("EXTRA BALL"), CL("SHOOT AGAIN"), "", eNone, eNone, eBlink, 1000, True, "" : pupevent 819
...
Sub EndOfBallComplete()
    ...
    If((BallsRemaining(CurrentPlayer) <= 0)AND(BallsRemaining(NextPlayer) <= 0))Then
        ...
        EndOfGame() :Playsound "ThrillerEnd" : pupevent 859
...
Sub Drain_Hit()
    ...
    If(bGameInPLay = True)AND(Tilted = False)Then
        ...
        If(bBallSaverActive = True)Then
            ...
            DMD "_", CL("BALL SAVED"), "_", eNone, eBlinkfast, eNone, 800, True, "" : pupevent 824
        Else
...
Sub AddPlayfieldMultiplier(n)
    ...
    if(PlayfieldMultiplier(CurrentPlayer) + n <= MaxMultiplier)then
        ...
        DMD "_", CL("PLAYFIELD X " &NewPFLevel), "_", eNone, eNone, eNone, 2000, True, "fx_bonus" : pupevent 857
...
Sub AwardExtraBall()
    If NOT bExtraBallWonThisBall Then
        DMD "_", CL(("EXTRA BALL WON")), "_", eNone, eBlink, eNone, 1500, True, SoundFXDOF("fx_Knocker", 122, DOFPulse, DOFKnocker) : pupevent 819
        DOF 121, DOFPulse
        ExtraBallsAwards(CurrentPlayer) = ExtraBallsAwards(CurrentPlayer) + 1
...
Sub AwardJackpot() 'award a normal jackpot, double or triple jackpot
    Dim tmp
    DMD CL(FormatScore(Jackpot(CurrentPlayer))), CL("JACKPOT"), "d_border", eBlinkFast, eBlinkFast, eNone, 1000, True, "" : pupevent 855
    DOF 126, DOFPulse
    tmp = INT(RND * 2)
    Select Case tmp
...
Sub AwardSuperJackpot() 'this is actually 4 times a jackpot
    ...
    DMD CL(FormatScore(SuperJackpot)), CL("SUPER JACKPOT"), "d_border", eBlinkFast, eBlinkFast, eNone, 1000, True, "vo_superjackpot" : pupevent 856
    DOF 126, DOFPulse
...
Sub DMDScore()
    ...
    if(dqHead = dqTail)Then
        ...
        Select Case Battle(CurrentPlayer, 0)
            ...
            Case 4:tmp1 = CL("HIT ORBIT L-R " & 6-orbithits)
            Case 5:tmp1 = CL("CHASE THE LIGHTS")
            Case 6:tmp1 = CL("FOLLOWS THE LIGHTS ")
            Case 7:tmp1 = CL("HIT ALL TARGETS " & 20-TargetHits7)
            Case 8:tmp1 = CL("HIT YELLOW TARGETS " & 10-TargetHits8)
            Case 9:tmp1 = CL("HIT MASK TARGET " & 5-CaptiveBallHits)
            ...
            Case 15:tmp1 = CL("MYERS KILL MISSION" & "Ball " & Balls)
        End Select
        tmp2 = ""
    End If
...
Sub ShowTableInfo
    ...
    DMD "", CL("TRICK OR TREAT" &myversion), "", eNone, eNone, eNone, 2000, False, ""
...
Sub Trees_Timer()
    ...
    'Tree10.RotY = TreeDir *2
     citrouille.RotY = - TreeDir *9
    'Tree14.RotZ = TreeDir *5
    'Tree15.RotZ = - TreeDir *4
    'avion.RotZ = - TreeDir *10
     mike.RotY = - TreeDir *15
    'casque.RotX = - TreeDir *5
End Sub

Sub BoatTimer_Timer()
    BoatDir = SIN(BoatStep * MyPi)
    BoatStep = (BoatStep + 1)MOD 360
    'avion.Y = avion.Y + BoatDir * 6
    mike.X = mike.X + BoatDir * 3
End Sub
...
Sub Bumper3_Hit
    If NOT Tilted Then
        PlaySoundAt SoundFXDOF("fx_bumper3", 107, DOFPulse, DOFContactors), Bumper3
        DOF 137, DOFPulse
        ' add some points
        AddScore BumperValue(CurrentPlayer)
        If Battle(CurrentPlayer, 0) = 2 Then
...
Sub Target1_Hit
    PlaySoundAt SoundFXDOF("fouet", 115, DOFPulse, DOFTargets), Target1
    If Tilted Then Exit Sub
    AddScore 5000
...
Sub Target2_Hit
    PlaySoundAt SoundFXDOF("fouet", 115, DOFPulse, DOFTargets), Target2
    If Tilted Then Exit Sub
    AddScore 5000
    TargetBonus = TargetBonus + 1
    ' Do some sound or light effect
...
Sub Target3_Hit
    PlaySoundAt SoundFXDOF("fouet", 115, DOFPulse, DOFTargets), Target3
    If Tilted Then Exit Sub
    AddScore 5000
    TargetBonus = TargetBonus + 1
    ' Do some sound or light effect
...
Sub Check4BankTargets
     If light12.state + Light12a.state + light13.state + light14.state = 4 Then
        light12.state = 0
        light12a.state = 0
        light13.state = 0
        light14.state = 0
...
Sub spinner4_Spin
    If Tilted Then Exit Sub
    PlaySoundAt "fx_spinner", Spinner4
    DOF 124, DOFPulse
    Addscore spinnervalue(CurrentPlayer)
    Select Case Battle(CurrentPlayer, 0)
        Case 1
...
Sub Door_Hit
    PlaySoundAt "fx_door-creek", doorf
    OpenDoor
    Lightdoor.State = 2
End Sub
...
Sub lock_Hit
    ...
    If light27.State = 1 Then  'lock the ball
        ...
        Select Case BallsInLock(CurrentPlayer)
            Case 1:DMD "_", CL("BALL 1 LOCKED"), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_oneballlock" : pupevent 814
            Case 2:DMD "_", CL("BALL 2 LOCKED"), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_twoballlock" : pupevent 815
            Case 3:DMD "_", CL("BALL 3 LOCKED"), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_threeballlock" : pupevent 816
        End Select
        light27.State = 0
...
Sub StartMainMultiball
    AddMultiball 3
    DMD "_", CL("MULTIBALL"), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_multiball" : pupevent 831
    StartJackpots
    ChangeGi green
...
Sub Check5BankTargets
    ...
    If light15.state + light16.state + light17.state + light18.state + light19.state = 5 Then
        ...
        Else
            Light61.State = 1
            mystery.visible = 1 : pupevent 841
            AddScore 25000
        End If
        ' reset the lights
        light15.state = 0
...
Sub GiveRandomAward() 'from the Pyramid
    ...
    DMD CL("MICHAEL AWARD"), "", "", eNone, eNone, eNone, 50, False, "fx_spinner"
    ...
    Select Case tmp
        ...
        Case 19, 20, 21 'Complete Battle
            If Battle(CurrentPlayer, 0) > 0 AND Battle(CurrentPlayer, 0) < 13 Then
                DMD CL("MISSION"), CL("COMPLETED"), "", eBlink, eBlink, eNone, 1500, True, "fx_fanfare1" : pupevent 853
                WinBattle
            Else
                DMD CL("BIG POINTS"), CL("100000"), "", eBlink, eBlink, eNone, 1500, True, "fx_fanfare1"
                AddScore 100000
        ...
        Case 22, 23, 36, 37, 38 'PlayField multiplier
            DMD CL("INCREASED"), CL("PLAYFIELD X"), "", eBlink, eBlink, eNone, 1500, True, "fx_fanfare1" : pupevent 857
            AddPlayfieldMultiplier 1
...
Sub LeftRampDone_Hit
    ...
    if LastSwitchHit = "LeftRampDone" Then
        Addscore jackpot(CurrentPlayer)
        DMD CL("COMBO"), CL(jackpot(CurrentPlayer)), "_", eNone, eBlinkFast, eNone, 1000, True, "combosound" : pupevent 854
    End If
...
Sub RightRampDone_Hit
    ...
    If light31.State = 2 Then
        DMD CL("KILLER KNIFE"), CL(jackpot(CurrentPlayer)), "_", eNone, eBlinkFast, eNone, 1000, True, "vo_Jackpot"
        AddScore Jackpot(CurrentPlayer)
        LightEffect 2
        FlashEffect 2
    Else
...
Sub StartBattle
    ...
    Select Case NewBattle
        Case 1 'Serpent Yards = Super Spinners
            DMD CL("LINDA"), CL("SHOOT THE SPINNERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light45a.State = 2
            Light49.State = 2
            SpinCount = 0
        ...
        Case 2 'Calakmul = Super Pop Bumpers
            DMD CL("HOUSE MICHAEL"), CL("HIT THE POP BUMPERS"), "", eNone, eNone, eNone, 1500, True, ""
            Light22.State = 2
            Light22a.State = 2
            Light22b.State = 2
            sang.visible = 1
        ...
        Case 3 'Sierra de Chiapas = Ramps
            DMD CL("SAMUEL"), CL("SHOOT THE RAMPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits3 = 0
            pupevent 847
        Case 4 'Karnak = Orbits
            DMD CL("MASK"), CL("SHOOT THE ORBITS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            OrbitHits = 0
            pupevent 837
        Case 5 'CopÃ¡n = Shoot the lights 2
            DMD CL("THE HEDGE"), CL("SHOOT THE LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light45.State = 2
            Light46.State = 2
            Light47.State = 2
        ...
        Case 6 'The Pit = Shoot the lights 1
            DMD CL("LAURIE"), CL("SHOOT THE LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            pupevent 843
        Case 7 'Palenque=  Target Frenzy
            DMD CL("PUMPKIN"), CL("SHOOT BLOOD LIGHT"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqAllTargets.Play SeqRandom, 10, , 1000
            TargetHits7 = 0
            pupevent 838
        Case 8 'ChichÃ©n ItzÃ¡ = Left & Right Targets
            DMD CL("FRANK"), CL("SHOOT YELLOW TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            Light44.State = 2
            Light50.State = 2
            TargetHits8 = 0
            pupevent 845
        Case 9 'Bonampak = Captive Ball
            DMD CL("ANNIE"), CL("SHOOT MASK TARGET"), "", eNone, eNone, eNone, 1500, True, ""
            Light29.State = 2
            CaptiveBallHits = 0
            pupevent 844
        Case 10 'Monte Alban = Ramps and Orbits
            'uses the ramphits10 to count the hits
            DMD CL("KNIFE"), CL("SHOOT RAMPS AND LANE"), "", eNone, eNone, eNone, 1500, True, ""
            Light47.State = 2
            Light48.State = 2
            RampHits10 = 0
            pupevent 839
        Case 11 'Uxmal = Blue Targets
            DMD CL("TOMMY"), CL("SHOOT BLUE TARGETS"), "", eNone, eNone, eNone, 1500, True, ""
            LightSeqBlueTargets.Play SeqRandom, 10, , 1000
            TargetHits11 = 0
            pupevent 849
        Case 12 'Valley of the Jaguar = Super Loops
            DMD CL("FORD STATION"), CL("SHOOT THE LOOPS"), "", eNone, eNone, eNone, 1500, True, ""
            Light45.State = 2
            Light49.State = 2
            Gate2.Open = 1
            Gate3.Open = 1
        ...
        Case 13 'The Grand Cathedral = Follow the Lights 1
            DMD CL("RED RABBIT"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
        ...
        Case 14 'The City of the Gods = Follow the Lights 2
            DMD CL("DRIVE FORD"), CL("SHOOT LIT LIGHTS"), "", eNone, eNone, eNone, 1500, True, ""
            FollowTheLights.Enabled = 1
            AddMultiball 2
            StartJackpots
            ChangeGi green
        ...
        Case 15 'Mordekai the Summoner - the final battle
            DMD CL("MICHAEL"), CL("SHOOT THE JACKPOTS"), "", eNone, eNone, eNone, 1500, True, ""
            AddMultiball 4
            StartJackpots
            ChangeGi green
            pupevent 852
...
Sub WinBattle
    ...
    LightEffect 2
    GiEffect 2
    DMD "", CL("MISSION COMPLETED"), "_", eNone, eBlinkFast, eNone, 1000, True, "fx_Explosion01" : pupevent 853
    DOF 139, DOFPulse
    tmp = INT(RND * 4)
    Select Case tmp
...
Sub StopBattle2
    ...
    Select Case NewBattle
        Case 1:
        Case 2:Light22.State = 0:Light22a.State = 0:Light22b.State = 0:LightSeqBumpers.StopPlay : sang.visible = 0
        Case 3:
        Case 4:
        Case 5:
        Case 6:
