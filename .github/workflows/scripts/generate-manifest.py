...
table folder's git tree id) are intentionally absent here — everything else is
...
Examples:
    ...
    python generate-manifest.py tables/vpx-starwars/table.yml -o manifest.json

    # every table under tables/ (same discovery the release uses)
...
def resolve_files(tokens):
    ...
    "vpx-starwars", resolved under tables/). With no tokens, discover every
    tables/vpx-*/table.yml — the same set the release builds.
    """
    if not tokens:
        return sorted(str(p) for p in TABLES_DIR.glob("vpx-*/table.yml"))
    ...
    for token in tokens:
        ...
        elif (TABLES_DIR / token / "table.yml").is_file():
            files.append(str(TABLES_DIR / token / "table.yml"))
        else:
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        "Default: every table under tables/.",
    )
    parser.add_argument(
        "-o",
        "--output",
