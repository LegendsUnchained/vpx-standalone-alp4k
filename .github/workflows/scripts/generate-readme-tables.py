...
of the top-level ``README.md`` from the contents of ``tables/vpx-*/``.

Data sources per row:

- Wizard vs Manual classification: a table folder is *Wizard* when
  ``tables/<folder>/table.yml`` exists and is not explicitly ``enabled: false``
...
def format_row(row: dict) -> str:
    return (
        f"| [{row['display']}](tables/{row['folder']}) "
...
def discover_folders() -> list[Path]:
    if not TABLES_DIR.is_dir():
        return []
    return sorted(
        (p for p in TABLES_DIR.iterdir() if p.is_dir() and p.name.startswith("vpx-")),
