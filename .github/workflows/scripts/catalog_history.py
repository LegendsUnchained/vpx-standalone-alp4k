...
# Bump whenever CONTENT_FIELDS changes. A stored ledger carries the version its
# fingerprints were computed under; on a mismatch release_history re-baselines
# them instead of reporting every table as updated (see rebaseline).
FINGERPRINT_VERSION = 2
# configFingerprint, not configVersion: the former hashes only a table's
# functional files, so recompressed art or a README rewrite no longer counts as
# an update. configVersion still drives device re-download; these are separate
# signals on purpose.
CONTENT_FIELDS = ('configFingerprint', 'tableChecksum', 'tableVersion', 'backglassChecksum',
...
def table_renames(previous, current, cwd='.'):
    ...
    result = subprocess.run(['git', 'diff', '--name-status', '-M',
                             f'refs/tags/{previous}', f'refs/tags/{current}', '--', 'tables/*/table.yml'],
...
def advance(history, manifest, tag, published_at, aliases=None):
    ...
    for key, entry in manifest.items():
        ...
        ledger[key] = previous
    out.update(schemaVersion=1, fingerprintVersion=FINGERPRINT_VERSION,
               latestRelease=tag, publishedAt=published_at)
    return out


def rebaseline(history, manifest):
    """Recompute stored fingerprints under the current CONTENT_FIELDS.

    Changing what a fingerprint covers invalidates every stored one, which would
    otherwise land as "all ~300 tables updated" in the Wizard on the next
    release. Re-baselining adopts the current definition while leaving the dates
    alone, so a scheme change is invisible to users.

    Deliberately silent about content: a table that genuinely changed in the
    same release as a fingerprint-scheme change keeps its previous update date.
    Scheme changes are rare and deliberate, so verify that release has no real
    changes before shipping one.
    """
    if not history or history.get('fingerprintVersion') == FINGERPRINT_VERSION:
        return history
    out = copy.deepcopy(history)
    rebased = 0
    for key, record in out.get('tables', {}).items():
        entry = manifest.get(key)
        if not isinstance(entry, dict):
            continue  # removed table: it will re-fingerprint if reintroduced
        record['fingerprint'] = fingerprint(entry)
        rebased += 1
    out['fingerprintVersion'] = FINGERPRINT_VERSION
    print(f"Re-baselined {rebased} fingerprints to version {FINGERPRINT_VERSION}; "
          "update dates preserved.")
    return out
