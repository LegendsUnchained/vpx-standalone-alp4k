...
All you need to do is add a `buttons.ini` file to the root of your
`vpx-table` folder.

The `buttons.ini` file looks like this:
...
So for deadpool, we added a `buttons.ini` file to `external/vpx-deadpool`,
and edited it like so. While older launchers remain in the fleet, keep an
identical `launcher.cfg` copy beside it for compatibility.
